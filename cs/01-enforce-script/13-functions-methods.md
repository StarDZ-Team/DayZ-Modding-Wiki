...
- [Pozorovano ve skutecnych modech](#observed-in-real-mods)
- [Teorie vs praxe](#theory-vs-practice)
- [Caste chyby](#common-mistakes)
- [Tabulka rychle reference](#quick-reference-table)

---

## Function Declaration Syntaxe
...
### Multiple Return Hodnotas via out Parametry
...
### POZOR: JsonFileLoader Vraci void
...
3. **Vychozi values for class types are restricted to `null` or `NULL`:**
...
### Modifikator Reference

| Modifikator | Vyznam | Priklad |
|----------|---------|---------|
| `proto native` | Implemented in C++ engine code | `proto native void SetPosition(vector pos);` |
| `proto native owned` | Vraci a value the caller owns (manages memory) | `proto native owned string GetTyp();` |
...
- **Konstanty/lookups** --- `Init()` + `Cleanup()` for static data tables

### Singleton Vzor (Static + Instance)
...
### Workaround 3: Vychozi Parametry
...
### Kdy pouzit Threads (and When Not To)
...
> Vzors confirmed by studying professional DayZ mod source code.
...
| Vychozi parameter expressions | Should support function calls | Only literal values (`42`, `true`, `null`, `""`) are allowed; no expressions |
...
### 5. Using Expressions in Vychozi Parametry

Vychozi parameter values must be compile-time literals. Expressions, function calls, and variable references are not allowed.
...
## Tabulka rychle reference

| Funkce | Syntaxe | Poznamky |
|---------|--------|-------|
...
| Vychozi value | `void Fn(int x = 5)` | Literals only, no expressions |
