...
Calls `SoundStop()` on the effect and sets the reference to `null`. Vraci `false` if the sound was already null or on a dedicated server.
...
| Metoda | Popis |
|--------|-------------|
| `SoundPlay()` | Start playback. Vraci `bool` (success). |
| `SoundStop()` | Stop playback. Respects fade-out duration if set. |
| `IsPlaying()` | Vraci `true` if the sound is currently playing. |
...
Poznamka: `PlaySoundSet` / `StopSoundSet` on `Object` already include this guard internally, so you do not need to check when using those methods.
...
[Domu](../../README.md) | [<< Predchozi: Player System](14-player-system.md) | **Sound System**
