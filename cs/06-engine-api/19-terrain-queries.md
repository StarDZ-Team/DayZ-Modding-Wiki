...
The most commonly used terrain query. Vraci the Y (vertical) coordinate of the terrain at a given X,Z position. This ignores objects, roads, and water --- it returns raw terrain height only.
...
Vraci height including road surfaces (bridges, elevated roads). Use this when you need the actual walkable surface, not raw terrain.
...
### SurfaceGetTyp --- Material at Position

Vraci the surface material name at a given X,Z coordinate. The return value is the Y position where the surface was found.
...
Vraci the surface type and liquid type directly under a specific object.
...
Vraci the normal vector of the terrain surface, pointing away from the ground. Essential for aligning objects to slopes.
...
Vraci a noise multiplier for a surface at a given position, used by the stealth/sound system.
...
Vraci the water depth at a world-space position. Vraci 0 or negative if the position is above water.
...
| `iTyp` | `int` | Intersection mode (`ObjIntersectView`, etc.) |
| `radius` | `float` | Radius of the ray (0 = line, >0 = thick ray) |
| `flags` | `CollisionPriznaks` | What to report |

**Vraci:** `true` if the ray hit something.
...
### Obstruction Check (Vanilla Vzor)
...
- **Server/Client:** All terrain queries (`SurfaceY`, `SurfaceGetTyp`, `SurfaceGetNormal`, `SurfaceIsSea`, `SurfaceIsPond`) are safe to call on both server and client. World modification methods like `SetDate()` are server-authoritative.
...
- **Map Dependency:** `SurfaceGetTyp` returns different surface names depending on the map. Chernarus and Livonia share most surface type names (`cp_gravel`, `cp_concrete`, etc.), but custom maps may define their own. Always handle unknown surface types gracefully.
...
|--------------------------|-----------------|
| `SurfaceY` returns ground height | Vraci raw terrain height, ignoring roads, bridges, and objects. Use `SurfaceRoadY` for surfaces that include roads. |
...
| `GetObjectsAtPosition` returns all objects | Vraci objects with physics bodies. Pure visual objects (particles, effects) are not returned. |
...
| `CollisionPriznaks.ALLOBJECTS` returns everything | Vraci the first contact per object, not all contacts per object. Multiple results come from multiple distinct objects. |
...
| Vzor | Source | File/Location |
|---------|--------|---------------|
| `SurfaceY` snap for ground-level particle placement | Vanilla | `4_World/classes/contaminatedarea/effectarea.c` |
| `SurfaceGetTyp` for vehicle wheel surface detection | Vanilla | `4_World/entities/vehicles/carscript.c` |
...
[Domu](../../README.md) | [<< Predchozi: Animation System](18-animation-system.md) | **Terrain & World Queries**
