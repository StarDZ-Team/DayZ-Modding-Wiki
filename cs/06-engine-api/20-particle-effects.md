...
**Dulezite:** Particles belonging to a `ParticleManager` pool ignore auto-destroy flags --- the pool manages their lifecycle.
...
**Poznamka:** On legacy `Particle`, wiggle only works when the particle has a parent. On `ParticleSource`, it works in all cases.
...
## ParticleProperties and Priznaks
...
### ParticlePropertiesPriznaks

| Priznak | Popis |
|------|-------------|
| `NONE` | Vychozi, no special behavior |
...
Vzors seen in vanilla DayZ and community mods:
...
| `ParticleAutoDestroyPriznaks` controls particle lifetime | Ignored for particles managed by a `ParticleManager` pool --- the pool handles lifecycle |
...
### 3. Not Destroying EffectParticle Reference
...
- **Performance:** The global `ParticleManager` pool is limited to 10,000 slots (`ParticleManagerKonstanty.POOL_SIZE`). Exceeding this creates "virtual" particles that wait for a slot to free up. Mods spawning many simultaneous particles (e.g., weather effects, contaminated areas with hundreds of emitters) should monitor pool usage and avoid exhausting it.
...
[Domu](../../README.md) | [<< Predchozi: Terrain & World Queries](19-terrain-queries.md) | **Particle & Effect System**
