...
> **Performance Varovani:** The maximum number of emitters is clamped to 1000 per zone. More emitters means worse performance. Keep `InnerPartDist` large enough to avoid exceeding this limit.
...
> **Poznamka:** The JSON snippets below include comments for clarity. Real JSON files must not contain comments.
...
| Typ | Breadcrumbs? | EyeAccommodation | Ucel |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | Konstanta darkness deep underground. Eye accommodation is fixed at the configured value |
...
1. **Script weather state machine** --- override `WorldData::WeatherOnBeforeZmena()` (see `4_World/Classes/Worlds/Enoch.c` for an example)
...
### Root Prvek Attributes

| Attribute | Typ | Vychozi | Popis |
|-----------|------|---------|-------------|
...
> **Poznamka:** You only need to include the phenomena you want to override. Omitted phenomena use engine defaults.
...
| float[] | `wetnessWeightModifikators` | `[1.0, 1.0, 1.33, 1.66, 2.0]` | Item weight multipliers by wetness level: Dry, Damp, Wet, Soaked, Drenched |
...
| Typ | Parametr | Vychozi | Popis |
|------|-----------|---------|-------------|
| float | `sprintStaminaModifikatorErc` | `1.0` | Stamina consumption rate during standing sprint |
| float | `sprintStaminaModifikatorCro` | `1.0` | Stamina consumption rate during crouched sprint |
...
| float | `sprintSwimmingStaminaModifikator` | `1.0` | Stamina consumption during fast swimming |
| float | `sprintLadderStaminaModifikator` | `1.0` | Stamina consumption during fast ladder climbing |
| float | `meleeStaminaModifikator` | `1.0` | Stamina consumed by heavy melee and evasion |
| float | `obstacleTraversalStaminaModifikator` | `1.0` | Stamina consumed by jumping, climbing, vaulting |
| float | `holdBreathStaminaModifikator` | `1.0` | Stamina consumption when holding breath |
...
| bool | `allowRefillSpeedModifikator` | `true` | Allow ammo-type-based shock recovery modifier |
...
| Parametr | Vychozi | What It Disables |
|-----------|---------|------------------|
...
| `disallowedTypsInUnderground` | `["FenceKit", "TerritoryPriznakKit", "WatchtowerKit"]` | Item types prohibited underground (includes inherited) |
...
Priklad: `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
To process `customString`, override `OnSpawnByObjectSpawner()` on the spawned item's script class. See `StaticPriznakPole` in vanilla scripts for an example where the custom string specifies which flag to spawn on the pole.

> **Performance Varovani:** Spawning a large number of objects through this system impacts both server and client performance. Use it for detail objects and small additions, not for large-scale world modifications.
...
- **Overrides the script state machine.** If both `cfgweather.xml` and a scripted `WeatherOnBeforeZmena()` override are active, the XML limits apply and the script operates within those limits.
- **Only one cfgweather.xml is loaded** per mission folder. Multiple weather mods cannot stack XML files.

### Gameplay Settings

- **One cfgGameplay.json per mission.** Hodnotas are global --- they affect all players on the server.
...
[<< Predchozi: Admin & Server Management](22-admin-server.md) | **World Systems**
