...
Dieses Kapitel behandelt the complete workflow for adding interactive doors and climbable ladders to custom building models, based on official Bohemia Interactive documentation.
...
1. **Named selections across all relevant LODs.** The geometry that represents the door muss assigned to a named selection (e.g., `door1`) in each of these LODs:
   - **Resolution LOD** -- the visual mesh der Spieler sees.
   - **Geometry LOD** -- the physical collision shape. Must also contain a named property `class` with der Wert `house`.
   - **View Geometry LOD** -- used for visibility checks and action ray-casting. The selection name here corresponds to the `component` parameter in das Spiel config.
...
Definiert the animated bones. Each door gets a bone entry. Bones are listed as pairs: the bone name followed by its parent (empty string `""` for root-level bones).
...
Definiert the animations for each bone. Die Klasse name under `CfgModels` **must match your model's filename** (without extension) for the link to work.
...
| `selection` | The named selection in the model that sollte animated. |
| `source` | Links to das Spiel config's `Doors` class. Must match die Klasse name in `config.cpp`. |
...
| `minValue` / `maxValue` | Animation phase range. Typischerweise `0` to `1`. |
...
Das Spiel config connects the animated model to game systems -- sounds, damage, and door state logic. The config class name **must** follow the pattern `land_modelname` to link correctly with the model.
...
- In the **Memory LOD**, the action point muss **shared** between the two wings -- use one named selection and one vertex for the action position.
...
For doors that slide along a track rather than swinging (such as barn doors or sliding panels), change the animation `type` in `model.cfg` from `"rotation"` to `"translation"`. The axis vertices in the Memory LOD then define the direction of travel stattdessen of the pivot line.
...
Unlike doors, ladders in DayZ require **no animation config** and **no special game config entries** beyond die Basis building class. The entire ladder setup is done through Memory LOD vertex placement and one View Geometry selection. This makes ladders simpler to set up than doors, but the vertex placement muss precise.

### Supported Ladder Types

DayZ supports two types of ladders:

1. **Front bottom enter with side-way top exit** -- Der Spieler approaches from the front at the bottom and exits to the side at the top (against a wall).
2. **Front bottom enter with front top exit** -- Der Spieler approaches from the front at the bottom and exits forward at the top (onto a roof or platform).
...
|----------------|-------------|
| `ladderN_bottom_front` | Definiert the bottom entry step -- where der Spieler begins climbing. |
| `ladderN_middle_left` | Definiert a middle entry/exit point (left side). Can contain multiple vertices if the ladder passes multiple floors. |
| `ladderN_middle_right` | Definiert a middle entry/exit point (right side). Can contain multiple vertices for multi-floor ladders. |
| `ladderN_top_front` | Definiert the upper exit step -- where der Spieler finishes climbing (front exit type). |
| `ladderN_top_left` | Definiert the upper exit direction for wall-mounted ladders (left side). Must be at least **5 ladder steps higher** than the floor (approximately the height of a standing player on a ladder). |
| `ladderN_top_right` | Definiert the upper exit direction for wall-mounted ladders (right side). Same height requirement as `top_left`. |
| `ladderN` | Definiert where the "Enter Ladder" action widget appears to the player. |
| `ladderN_dir` | Definiert the direction from which the ladder kann climbed (approach direction). |
| `ladderN_con` | The measurement point for the enter action. **Must be placed at floor level.** |
| `ladderN_con_dir` | Definiert the direction of a 180-degree cone (originating from `ladderN_con`) within which the action to enter the ladder is available. |
...
In addition to the Memory LOD setup, you must create a **View Geometry** component with a named selection called `ladderN`. This selection must cover the **entire volume** of the ladder -- the full height and width of the climbable area. Without this View Geometry selection, the ladder wird nicht function correctly.
...
Characters **collide with geometry while climbing a ladder**. Das bedeutet you must ensure there is enough clear space around the ladder for the climbing character in both:
...
Unlike the Arma series, DayZ does **not** require a `ladders[]` array in das Spiel config class. Jedoch, two things are still necessary:
...
2. The **Geometry LOD** must contain the named property `class` with der Wert `house`.
...
| **Roadway LOD** | Surface interaction for characters. | Not typischerweise required. | Sufficient clearance around the ladder for climbing characters. |

### Named Selection Consistency

A critical requirement is that **named selections muss consistent across all LODs** that reference them. If a selection is called `door1` in the Resolution LOD, it must also be `door1` in the Geometry, Fire Geometry, and View Geometry LODs. Mismatched names between LODs will cause the door or ladder to fail silently.
...
7. **Keep one `model.cfg` per model or folder.** The `model.cfg` macht nicht need to sit next to the `.p3d` file, but keeping them close makes organization easier. It can also be placed higher in the folder structure to cover multiple models.
...
9. **Re-binarize terrain after adding building configs.** Adding `class=house` and a game config class means model paths in `.wrp` files are replaced with class references. Your terrain muss re-binarized for this to take effect.

10. **Update the navmesh after placing buildings.** Rebuilt terrain without an updated navmesh can cause AI to walk through doors stattdessen of using them properly.
...
| Mistake | Symptom | Fix |
|---------|---------|-----|
| `CfgModels` class name macht nicht match model filename. | Door animation macht nicht play. | Rename die Klasse to match the `.p3d` filename exactly (without extension). |
...
| Axis vertices missing or only one vertex defined. | Door pivots from the wrong point or macht nicht rotate at all. | Place exactly two vertices in the Memory LOD for the axis selection (e.g., `door1_axis`). |
| `source` in `model.cfg` macht nicht match class name in `config.cpp` Doors. | Door ist nicht linked to game logic -- no sounds, no state changes. | Ensure the `source` parameter and the Doors class name are identical. |
| Forgetting `class = "house"` named property in Geometry LOD. | Building ist nicht recognized as an interactive structure. | Add the named property in Object Builder's Geometry LOD. |
...
| Mistake | Symptom | Fix |
|---------|---------|-----|
| `ladderN_con` not placed at floor level. | "Enter Ladder" action macht nicht appear or appears at the wrong height. | Move the vertex to ground/floor level. |
| Missing View Geometry selection `ladderN`. | Ladder kann nicht interacted with. | Create a View Geometry component with a named selection covering the full ladder volume. |
| `ladderN_top_left` / `ladderN_top_right` too low. | Character clips through the wall or floor at the top exit. | These muss at least 5 ladder steps higher than the floor level. |
...
| Ladder numbering starts at 0. | Ladder macht nicht function. | Numbering starts at `1` (`ladder1_`, not `ladder0_`). |
| Specifying `ladders[]` in game config. | Wasted effort (harmless but unnecessary). | DayZ macht nicht use the `ladders[]` array. Remove it and rely on Memory LOD vertex placement. |
