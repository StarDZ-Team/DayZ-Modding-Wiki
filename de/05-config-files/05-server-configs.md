...
The `init.c` script is the first thing der Server executes. It initializes the Central Economy and creates the mission instance.
...
Multiple tags kann combined. No `usage` tags = item wird nicht spawn. No `value` tags = spawns in all tiers.

### Disabling an Item

Set `nominal=0` and `min=0`. Der Gegenstand never spawns but can still exist through scripts or crafting.

---

## cfgspawnabletypes.xml --- Attachments and Cargo

Steuert what spawns **already attached to or inside** other items.
...
Definiert named item pools referenced by `cfgspawnabletypes.xml`:
...
Befindet sich bei `db/globals.xml`, this file sets global CE parameters:
...
| `InitialSpawn` | 100 | Percentage of nominal spawned beim Start |
...
Loaded only when `enableCfgGameplayFile = 1` in `serverDZ.cfg`. Without this, die Engine uses hardcoded defaults.
...
`-mod=` mods muss installed by clients. `-servermod=` mods run server-side only.
...
Any `category`, `usage`, or `value` used in `types.xml` muss defined here:
...
Steuert which CE subsystems are active:
...
Flags: `init` (spawn beim Start), `load` (load persistence), `respawn` (respawn after cleanup), `save` (persist to database).

### Script-Side Economy Interaction

Items created via `CreateInInventory()` are automatisch CE-managed. For world spawns, use ECE flags:
...
Using a `usage` or `value` in types.xml that ist nicht defined in cfglimitsdefinition.xml causes der Gegenstand to silently fail to spawn. Check RPT logs for warnings.
...
Items whose class hierarchy macht nicht trace to a registered root class in `cfgeconomycore.xml` will never spawn, even with correct types.xml entries.

### cfggameplay.json Not Enabled

The file wird ignoriert unless `enableCfgGameplayFile = 1` is set in `serverDZ.cfg`.
...
| `restock=0` means instant respawn | Items reappear immediately after despawn | The CE batch processes restocking in cycles (typischerweise every 30-60 seconds), so there is always a delay regardless of the restock value |
| `cfggameplay.json` controls all gameplay | All tuning goes here | Many gameplay values are hardcoded in script or config.cpp and kann nicht overridden by cfggameplay.json |
...
| Multiple types.xml files merge cleanly | CE reads all registered files | Files muss registered in cfgeconomycore.xml via `<ce folder="custom">` directives. Simply placing extra XML files in `db/` macht nichthing |
