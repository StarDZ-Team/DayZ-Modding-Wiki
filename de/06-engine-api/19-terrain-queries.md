...
The `3D` variant searches a sphere stattdessen of a cylinder, respecting vertical distance.
...
> - Cache results; mache nicht call every frame
...
Raycasting traces a line (or thick line) through die Welt and reports what it hits. DayZ provides several raycast methods on the static `DayZPhysics` class, each suited to different use cases.
...
Steuert what the raycast reports. Definiert in `1_core/proto/endebug.c`:
...
Definiert which physics layers participate in bullet-type raycasts. Definiert in `3_game/global/dayzphysics.c`:
...
| `contactComponent` | `out int` | Index of hit component on das Objekt |
| `results` | `set<Object>` | Set of all objects hit (kann NULL) |
...
These use the physics interaction layer system stattdessen of geometry intersection modes. They are more appropriate for bullet trajectory simulation and physics-aware queries.
...
- **Combine `SurfaceIsSea` and `SurfaceIsPond` for water checks.** There is no single `SurfaceIsWater` function. Always check both unless you spezifisch need to distinguish between sea and pond.
...
| Passing `null` for `ignore` when der Spieler could self-intersect | Always pass der Spieler (or the casting entity) as `ignore` to prevent the ray from hitting the caster's own collision geometry. |
