...
Dieses Kapitel behandelt the complete particle pipeline: the `ParticleList` registry, both creation approaches, the `EmitorParam` system for runtime tuning, the `EffectParticle` wrapper, `SEffectManager` integration, and real-world patterns from vanilla code.
...
`ParticleManager` uses a pre-allocated pool of `ParticleSource` entities. This avoids the overhead of creating and destroying objects zur Laufzeit.
...
2. The particle is attached to der Elternteil (if any) with the cached position and orientation.
...
Every Effect played through `SEffectManager` is automatisch registered and receives an integer ID. The manager holds a `ref` to the Effect, preventing garbage collection.
...
`.ptc` files are binary particle definitions created with the **Enfusion Workbench Particle Editor**. They define emitters, textures, blend modes, velocities, colors, and all visual properties. These files kann nicht authored from script alone --- they require the toolchain.
...
| `EffectParticle.ForceParticleRotationRelativeToWorld()` kann called anytime | Only takes effect on the next `Start()` call, cannot live-update an active particle |
| `SetSource()` can change the particle ID zur Laufzeit | On legacy `Particle`, this only takes effect after stopping and replaying; `ParticleSource` updates immediately |
...
Both `Particle` and `ParticleSource` guard against null internally, but calling methods on a particle that hat nicht been played yet (or has already been stopped and cleaned up) macht nichthing silently.
...
- **Prefer `SetAutodestroy(true)` for one-shot effects.** Fire-and-forget particles (explosions, blood splatters) should self-clean. Persistent effects (engine smoke, bleeding) sollte managed explicitly.
...
- **Multi-Mod:** `ParticleList` is a single global class. Multiple mods can register particles via `modded class ParticleList`, but particle filenames muss unique --- duplicates cause an error on the second registration, and `GetParticleIDByName()` only returns the first match.
...
- **Server/Client:** All particle rendering is client-side. Server-side particle effecters (`ParticleEffecter`) are network-synced entities that trigger client-side rendering via `OnVariablesSynchronized`. Direct `Particle` or `ParticleManager` calls on a dedicated server mache nichthing.
- **Legacy Compatibility:** The legacy `Particle` static methods (`Particle.PlayOnObject`, `Particle.CreateInWorld`) still work and are used by older mods. They sind nicht deprecated but are less efficient than `ParticleManager` equivalents.
