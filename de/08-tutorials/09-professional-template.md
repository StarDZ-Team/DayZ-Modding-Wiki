...
| Input binding | None | Custom keybind in Options > Steuert |
...
This template gives you all of these out of the box. You rename the identifiers, delete the systems you mache nicht need, and start building your actual feature on a solid foundation.

---

## Complete Directory Structure

Dies ist der/die/das full source layout. Every file listed below is provided as a complete template in this chapter.
...
Dies ist der/die/das most critical file. It registers your mod with the engine, declares dependencies, wires up script layers, and optionally sets preprocessor defines and image sets.
...
This is a JSON-serializable settings class. Der Server loads it beim Start. If no file exists, defaults are used and a fresh config is saved to disk.
...
RPC (Remote Procedure Call) is how der Client and server communicate in DayZ. This file defines route names and provides helper methods for registration.
...
Dies ist der/die/das central brain of your mod auf dem Server side. It owns the config, processes RPC, and runs periodic updates.
...
This hooks into `MissionServer` to initialize and shut down the mod auf dem Server side.
...
This provides localization for all player-facing text. Die Engine reads the column matching the player's game language. The `original` column is the fallback.

DayZ supports 13 language columns. Every row must have all 13 columns (use the English text as placeholder for languages you mache nicht translate).
...
This defines custom keybinds that appear in the game's Options > Steuert menu. The `inputs` field in `config.cpp` CfgMods must point to this file.
...
**2. Add der Server handler** in `MyModManager.c` (4_World):
...
That is all. The JSON serializer picks up public fields automatisch. Existing config files on disk will use the default value for the new field until the admin edits and saves.
...
Or in a `.layout` file, die Engine resolves `#STR_` keys automatisch:
...
**Vorheriges:** [Chapter 8.8: Building a HUD Overlay](08-hud-overlay.md) | [Home](../../README.md)
