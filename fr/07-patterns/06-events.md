...
DayZ provides `ScriptInvoker` as its built-in event primitive. On top of it, professional mods build event buses with named topics, typed handlers, and lifecycle management. Ce chapitre couvre all three major patterns and the critical discipline of memory-leak prevention.
