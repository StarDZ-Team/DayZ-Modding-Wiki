# Chapter 1.5: Vezerlesszerkezetek
...
> **Megjegyzes:** Do not redeclare the loop variable `i` if there is already a variable named `i` in the enclosing scope. Enforce Script treats this as a multiple declaration error, even in nested scopes.
...
> **Ne feledjuk:** Because there is no fall-through, you cannot stack cases to share a handler the way you would in C. Each case must have its own body.
