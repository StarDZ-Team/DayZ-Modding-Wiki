# Chapter 1.9 — Tipuskenyszerites es reflekszio

> **Cel:** Master safe type casting, runtime type checks, and Enforce Script's reflection API for dynamic property access.
...
- [Real-World Examples](#real-world-examples)
  - [Finding All Jarmuvek in the World](#finding-all-vehicles-in-the-world)
...
| Funkcio | `Class.CastTo` | `Type.Cast` |
|---------|----------------|-------------|
...
**Okoszabaly:** Use `Class.CastTo` when you branch on success/failure. Use `Type.Cast` when you just need the typed reference and will null-check later.
...
**Fontos:** `IsKindOf` checks the full inheritance chain, just like `IsInherited`. A `Mag_STANAG_30Rnd` returns `true` for `IsKindOf("Magazine_Base")`, `IsKindOf("InventoryItem")`, `IsKindOf("EntityAI")`, etc.

### IsInherited vs IsKindOf

| Funkcio | `IsInherited(typename)` | `IsKindOf(string)` |
|---------|------------------------|---------------------|
...
> **Megjegyzes:** `typename.Spawn()` only works for classes with a parameterless constructor. For DayZ entities, use `GetGame().CreateObject()` or `CreateObjectEx()`.
...
> **Figyelmezetes:** `GetClassVar`/`SetClassVar` silently fail if the variable name is wrong or the type doesn't match. Always validate variable names before use.

---

## Valos peldak

### Finding All Jarmuvek in the World
...
| Elozo | Fel | Kovetkezo |
|----------|----|------|
| [1.8 Memoriakezelees](08-memory-management.md) | [Part 1: Enforce Script](../README.md) | [1.10 Enumok es preprocesszor](10-enums-preprocessor.md) |
