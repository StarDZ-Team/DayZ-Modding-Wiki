# Chapter 1.10 — Enumok es preprocesszor

> **Cel:** Understand enum declarations, enum reflection tools, bitflag patterns, constants, and the preprocessor system for conditional compilation.
...
> **Megjegyzes:** Enum inheritance is useful for extending vanilla enums in modded code without changing the original.
...
> **Figyelmezetes:** Enforce Script does **not** validate enum assignments. Assigning an out-of-range integer to an enum variable compiles and runs without error.
...
> **Megjegyzes:** There is no `const` for reference types (objects). You cannot make an object reference immutable.
...
> **Megjegyzes:** Enforce Script `#define` only creates existence flags. It does **not** support macro substitution (no `#define MAX_HP 100` — use `const` instead).
