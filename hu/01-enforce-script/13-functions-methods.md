...
- [Function Declaration Szintaxis](#function-declaration-syntax)
  - [Standalone Functions](#standalone-functions)
  - [Instance Metoduss](#instance-methods)
  - [Static Metoduss](#static-methods)
- [Parameter Passing Modes](#parameter-passing-modes)
  - [By Ertek (Alapertelmezett)](#by-value-default)
  - [out Parameterek](#out-parameters)
  - [inout Parameterek](#inout-parameters)
...
- [Metodus Overloading (Not Supported)](#method-overloading-not-supported)
- [The event Keyword](#the-event-keyword)
- [Thread Metoduss (Coroutines)](#thread-methods-coroutines)
- [Deferred Calls with CallLater](#deferred-calls-with-calllater)
- [Legjobb gyakorlatok](#best-practices)
- [Valos modokban megfigyelt](#observed-in-real-mods)
- [Elmelet vs gyakorlat](#theory-vs-practice)
- [Gyakori hibak](#common-mistakes)
- [Gyors referenciatablazat](#quick-reference-table)

---

## Function Declaration Szintaxis
...
Static methods belong to the class itself, not to any instance. Call them via `ClassName.Metodus()`. They cannot access instance fields or `this`.
...
### Multiple Return Erteks via out Parameterek
...
### FIGYELJ: JsonFileLoader Visszaad void
...
## Alapertelmezett Parameter Erteks

Enforce Script supports default values for parameters. Parameterek with defaults must come **after** all required parameters.
...
### Alapertelmezett ertekek a vanillabol
...
3. **Alapertelmezett values for class types are restricted to `null` or `NULL`:**
...
## Proto Native Metoduss (Engine Bindings)
...
| Modosito | Jelentes | Pelda |
|----------|---------|---------|
| `proto native` | Implemented in C++ engine code | `proto native void SetPosition(vector pos);` |
| `proto native owned` | Visszaad a value the caller owns (manages memory) | `proto native owned string GetTipus();` |
...
## Static vs Instance Metoduss

### Mikor hasznald Static
...
- **Konstansok/lookups** --- `Init()` + `Cleanup()` for static data tables

### Singleton Minta (Static + Instance)
...
3. **`super.MetodusName()` calls the parent** --- use this to extend behavior rather than completely replace it.
...
## Metodus Overloading (Not Supported)
...
### Workaround 1: Different Metodus Names
...
### Workaround 3: Alapertelmezett Parameterek
...
### Mikor hasznald Threads (and When Not To)
...
| Params | variadic | Parameterek to pass to the function |
...
> Mintas confirmed by studying professional DayZ mod source code.
...
| `MetodusEx()` for extended signatures | Vanilla / Expansion Market | When an API needs more parameters, append `Ex` rather than breaking existing callers |
...
|---------|--------|---------|
| Metodus overloading | Standard OOP feature | Not supported; use `Ex()` suffix or default parameters instead |
...
| Alapertelmezett parameter expressions | Should support function calls | Only literal values (`42`, `true`, `null`, `""`) are allowed; no expressions |

---

## Gyakori hibak

### 1. Forgetting override When Replacing a Parent Metodus
...
### 2. Expecting out Parameterek to Be Pre-initialized
...
### 5. Using Expressions in Alapertelmezett Parameterek

Alapertelmezett parameter values must be compile-time literals. Expressions, function calls, and variable references are not allowed.
...
## Gyors referenciatablazat

| Funkcio | Szintaxis | Megjegyzesek |
|---------|--------|-------|
...
| Alapertelmezett value | `void Fn(int x = 5)` | Literals only, no expressions |
