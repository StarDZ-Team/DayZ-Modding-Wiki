...
Items inherit CE (Kozponti gazdasag) lifetime and persistence from their `types.xml` entry. Use `ECE_NOLIFETIME` flag when creating objects that should never despawn.
...
| Konstans | Flags | Felhasznalasi terrulet |
|----------|-------|----------|
