...
| `sound3DProcessingTipus` | string | Engine processing preset for 3D sounds. |
...
## PlaySoundSet / StopSoundSet --- Entity Convenience Metoduss
...
Calls `SoundStop()` on the effect and sets the reference to `null`. Visszaad `false` if the sound was already null or on a dedicated server.
...
| Metodus | Leiras |
|--------|-------------|
| `SoundPlay()` | Start playback. Visszaad `bool` (success). |
| `SoundStop()` | Stop playback. Respects fade-out duration if set. |
| `IsPlaying()` | Visszaad `true` if the sound is currently playing. |
...
Megjegyzes: `PlaySoundSet` / `StopSoundSet` on `Object` already include this guard internally, so you do not need to check when using those methods.
