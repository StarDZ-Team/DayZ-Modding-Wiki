...
## Particle Architecture Attekintes
...
### Metodus 1: ParticleManager (Recommended)
...
### Metodus 2: Particle Static Metoduss (Legacy)
...
**Fontos:** Particles belonging to a `ParticleManager` pool ignore auto-destroy flags --- the pool manages their lifecycle.
...
**Megjegyzes:** On legacy `Particle`, wiggle only works when the particle has a parent. On `ParticleSource`, it works in all cases.
...
### Key Metoduss for Particles

| Metodus | Visszaad | Leiras |
|--------|---------|-------------|
...
### ParticlePropertiesJelzos

| Jelzo | Leiras |
|------|-------------|
| `NONE` | Alapertelmezett, no special behavior |
...
Mintas seen in vanilla DayZ and community mods:
...
| `ParticleAutoDestroyJelzos` controls particle lifetime | Ignored for particles managed by a `ParticleManager` pool --- the pool handles lifecycle |
...
### 3. Not Destroying EffectParticle Hivatkozasok
