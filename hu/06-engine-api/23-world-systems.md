...
| `Tipus` | string | Class name of the EffectArea subclass to spawn (`ContaminatedArea_Static`) |
| `TriggerTipus` | string | Trigger class name (`ContaminatedTrigger`). Leave empty for no trigger |
...
> **Performance Figyelmezetes:** The maximum number of emitters is clamped to 1000 per zone. More emitters means worse performance. Keep `InnerPartDist` large enough to avoid exceeding this limit.

### Particle Mezos (Pre-1.28, Legacy)
...
| `PPERequesterTipus` | Post-process effect class applied to the player's camera (`PPERequester_ContaminatedAreaTint`) |
...
> **Megjegyzes:** The JSON snippets below include comments for clarity. Real JSON files must not contain comments.
...
| Tipus | Breadcrumbs? | EyeAccommodation | Cel |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | Konstans darkness deep underground. Eye accommodation is fixed at the configured value |
...
> **Tipp:** When using `UseRaycast: 1`, raise the breadcrumb position slightly off the floor (a few centimeters in the Y axis) to avoid the ray being blocked by the ground surface.
...
1. **Script weather state machine** --- override `WorldData::WeatherOnBeforeValtozas()` (see `4_World/Classes/Worlds/Enoch.c` for an example)
...
### Root Elem Attributes

| Attribute | Tipus | Alapertelmezett | Leiras |
|-----------|------|---------|-------------|
...
> **Megjegyzes:** You only need to include the phenomena you want to override. Omitted phenomena use engine defaults.
...
| Tipus | Parameter | Alapertelmezett | Leiras |
|------|-----------|---------|-------------|
...
| float[] | `wetnessWeightModositos` | `[1.0, 1.0, 1.33, 1.66, 2.0]` | Item weight multipliers by wetness level: Dry, Damp, Wet, Soaked, Drenched |
...
| Tipus | Parameter | Alapertelmezett | Leiras |
|------|-----------|---------|-------------|
| float | `sprintStaminaModositoErc` | `1.0` | Stamina consumption rate during standing sprint |
| float | `sprintStaminaModositoCro` | `1.0` | Stamina consumption rate during crouched sprint |
...
| float | `sprintSwimmingStaminaModosito` | `1.0` | Stamina consumption during fast swimming |
| float | `sprintLadderStaminaModosito` | `1.0` | Stamina consumption during fast ladder climbing |
| float | `meleeStaminaModosito` | `1.0` | Stamina consumed by heavy melee and evasion |
| float | `obstacleTraversalStaminaModosito` | `1.0` | Stamina consumed by jumping, climbing, vaulting |
| float | `holdBreathStaminaModosito` | `1.0` | Stamina consumption when holding breath |
...
| bool | `allowRefillSpeedModosito` | `true` | Allow ammo-type-based shock recovery modifier |
...
| Parameter | Alapertelmezett | What It Disables |
|-----------|---------|------------------|
...
| `disallowedTipussInUnderground` | `["FenceKit", "TerritoryJelzoKit", "WatchtowerKit"]` | Item types prohibited underground (includes inherited) |
...
Pelda: `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
To process `customString`, override `OnSpawnByObjectSpawner()` on the spawned item's script class. See `StaticJelzoPole` in vanilla scripts for an example where the custom string specifies which flag to spawn on the pole.

> **Performance Figyelmezetes:** Spawning a large number of objects through this system impacts both server and client performance. Use it for detail objects and small additions, not for large-scale world modifications.
...
- **Leave `TriggerTipus` empty** if you want a visual-only gas cloud with no player damage.
...
- **Overrides the script state machine.** If both `cfgweather.xml` and a scripted `WeatherOnBeforeValtozas()` override are active, the XML limits apply and the script operates within those limits.
- **Only one cfgweather.xml is loaded** per mission folder. Multiple weather mods cannot stack XML files.

### Gameplay Settings

- **One cfgGameplay.json per mission.** Erteks are global --- they affect all players on the server.
...
[<< Elozo: Admin & Server Management](22-admin-server.md) | **World Systems**
