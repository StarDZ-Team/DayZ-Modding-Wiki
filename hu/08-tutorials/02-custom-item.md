# Chapter 8.2: Creating a Egyeni targy

> **Osszefoglalas:** This tutorial walks you through adding a brand-new item to DayZ. You will define the item in config.cpp, give it textures using hidden selections, add it to the server's spawn table, create a string table for its display name, and test it in-game. By the end, you will have a custom item that players can find, pick up, and carry in their inventory.
...
- [Step 2: Set Up Hidden Selections for Texturak](#step-2-set-up-hidden-selections-for-textures)
- [Step 3: Create Basic Texturak](#step-3-create-basic-textures)
...
- [Step 7: Polish -- Model, Texturak, and Sounds](#step-7-polish----model-textures-and-sounds)
...
- A text editor
- DayZ eszkozok installed (for texture conversion, optional)
...
Items in DayZ are defined in the `CfgJarmuvek` config class. Despite the name "Jarmuvek", this class holds ALL entity types: items, buildings, vehicles, animals, and everything else.
...
| `hiddenSelectionsTexturak[]` | Path to `.paa` | Your custom texture for each hidden selection. |
...
## Step 2: Set Up Hidden Selections for Texturak
...
3. `hiddenSelectionsTexturak[]` provides your replacement textures, in matching order
...
The first entry in `hiddenSelectionsTexturak` replaces the first entry in `hiddenSelections`. If you had multiple selections:
...
1. Open **Object Builder** (from DayZ eszkozok)
...
For initial testing, you can point `hiddenSelectionsTexturak` to a vanilla texture instead of creating your own:
...
2. **Convert to `.paa`:**
   - Open **TexView2** from DayZ eszkozok
   - Go to **File > Open** and select your `.tga` or `.png`
   - Go to **File > Save As** and save as `.paa` format
   - Save it to `MyFirstMod/Data/Texturak/field_journal_co.paa`
...
| `_de` | Reszlet | Additional detail overlay |
...
| `min` | `5` | Minimum number the Kozponti gazdasag tries to maintain |
...
## Step 7: Polish -- Model, Texturak, and Sounds
...
- **Texture path wrong:** Verify that `hiddenSelectionsTexturak[]` points to the correct `.paa` file path. Paths use backslashes in config.cpp.
...
**Elozo:** [Chapter 8.1: Az elso modod (Hello World)](01-first-mod.md)
**Kovetkezo:** [Chapter 8.3: Building an Admin panel Module](03-admin-panel.md)
