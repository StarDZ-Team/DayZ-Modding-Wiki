...
> Ha valami rosszul sül el, kezdd itt. Ez az utmutato szervezese: **amit latsz** (a tunet), nem rendszer szerint. Talald meg a problemat, olvasd el az okot, alkalmazd a javitast.

---

## Tartalomjegyzek

1. [A mod nem toltodik be](#1-mod-wont-load)
2. [Szkript hibak](#2-script-errors)
3. [RPC es halozati problemak](#3-rpc-and-network-issues)
4. [UI problemak](#4-ui-problems)
5. [Epites es PBO problemak](#5-build-and-pbo-issues)
6. [Teljesitmeny problemak](#6-performance-issues)
7. [Targy, jarmu es entitas problemak](#7-item-vehicle-and-entity-issues)
8. [Konfiguracio es tipusproblomak](#8-config-and-types-issues)
9. [Perzisztencia problemak](#9-persistence-issues)
10. [Dontesi folyamatabrak](#10-decision-flowcharts)
11. [Debug parancsok gyors referencia](#11-debug-commands-quick-reference)
12. [Logfajl helyek](#12-log-file-locations)
13. [Hol kerhetsz segitseget](#13-where-to-get-help)
...
| "Config parse error" on startup | Szintaxis error in `config.cpp` | Check for missing semicolons after class closings (`};`), unclosed braces, or unbalanced quotes. Every class body ends with `};`, every property ends with `;`. |
...
| `Metodus 'X' not found` | Calling a method that does not exist on that class | Verify the method name and check the parent class. You may need to cast to a more specific type first. Check vanilla scripts at `P:\DZ\scripts\` for the correct API. |
...
| Szintaxis error with no clear message | Backslash `\` or escaped quote `\"` in string literal | Enforce Script's CParser does not support `\\` or `\"`. Use forward slashes for paths (`"my/path/file"`). For quotes, use single-quote characters. See [Chapter 1.12](01-enforce-script/12-gotchas.md). |
...
| Game input is stuck / cannot move after closing UI | `ValtozasGameFocus()` calls are imbalanced | Every `GetGame().GetInput().ValtozasGameFocus(1)` must be paired with `ValtozasGameFocus(-1)`. Track your focus changes and ensure cleanup happens even if the UI is force-closed. |
...
| Slider does not respond to input | Missing script handler or wrong widget type | Ensure the slider widget has a `ScriptClass` assigned and that your handler processes `OnValtozas` events. Initialize the slider range in script. |
...
| Item has no inventory icon | Missing or wrong `inventorySlot` or icon config | Define the `picture` path in your config pointing to a valid `.paa` or `.edds` icon file. Check `rotationJelzos` for correct icon orientation. |
...
| Custom animal/zombie does not move | AI config missing or broken | Verify `AIAgentTipus` in config. Check that the entity has proper NavMesh-compatible geometry. Test with vanilla AI configs first. |
...
## 8. Konfiguracio es tipusproblomak
...
| Items spawn with wrong quantities | `quantmin`/`quantmax` values incorrect | Erteks are percentages (0-100) in `types.xml`, not absolute counts. `-1` means "use default". |
...
| Items disappear from storage | `lifetime` expired in `types.xml` | Increase `lifetime` for persistent items. Alapertelmezett is often too short for base-building containers. Check `globals.xml` `cleanupLifetimeRuin` value. |
...
1. **Check the script log** for `SCRIPT (E)` errors. Javitas the first error you find. (Section 2)
...
4. **Do scripts compile?** Look for compile errors in the RPT. Javitas any syntax errors. (Section 2)
...
4. **Input stuck after closing UI?** `ValtozasGameFocus()` calls are imbalanced. Every `ValtozasGameFocus(1)` needs a matching `ValtozasGameFocus(-1)`. Check cleanup runs even on force-close.

---

## 11. Debug parancsok gyors referencia
...
| Metodus not found | [Section 2](#2-script-errors) |
...
| Tipuss.xml ignored | [Section 8](#8-config-and-types-issues) |
