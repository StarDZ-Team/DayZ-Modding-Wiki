...
**Modded classes are the single most important concept in DayZ modding.** Sono the mechanism that allows your mod to change the comportamento of existing game classes without replacing the original files. Without modded classes, DayZ modding as we know it would not exist.

Every major DayZ mod --- Community Online Tools, VPP Admin Tools, DayZ Expansion, Trader mods, medical overhauls, building systems --- works by using `modded class` to hook into vanilla classes and add or change comportamento. When you mod `PlayerBase`, every player in the game gets your new comportamento. When you mod `MissionServer`, your code runs as part of the server's mission lifecycle. When you mod `ItemBase`, every item in the game is affected.

This chapter is intentionally the longest and most detailed in Part 1 perche' getting modded classes right is what separates a working mod from one that crashes servers or breaks other mods.

---

## Come Funzionano le Classi Moddate

### L'Idea di Base

Normally, `class Child extends Parent` creates a new class named `Child` that inherits from `Parent`. But `modded class Parent` does something fundamentally different: it **replaces** the original `Parent` class in the engine's class hierarchy, inserting your code into the catena di ereditarieta'.
...
Questo e' tutto. No `extends`, no new name. The `modded` keyword tells the engine: "I am modifying the existing class `ClassName`."
...
The real power of modded classes is that **multiple mods can modify the same class**, and they all chain together automatically. The engine processes mods in ordine di caricamento, and each `modded class` inherits from the previous one.
...
Questo e' why **always calling `super`** is critical. If Mod A does not call `super.Say()`, then the original `Say()` never runs. If Mod B does not call `super.Say()`, then Mod A's `Say()` never runs. One mod skipping `super` breaks the entire chain.
...
## Cosa Puoi Fare in una Classe Moddata

### 1. Override Existing Methods

The most common use. Add comportamento before or after the vanilla code.
...
### 4. Accesso Private Members of the Original Class

Unlike normal inheritance where `private` members are inaccessible, **modded classes CAN accesso private members** of the original class. Questo e' a special rule of the `modded` keyword.
...
Questo e' powerful but should be used carefully. Private members are private for a reason --- they may change between DayZ updates.
...
## Target Comuni delle Classi Moddate

These are the classes that virtually every DayZ mod hooks into. Understanding what each one offers is essential.

### MissionServer

Runs on the server dedicato. Handles server startup, player connections, and the game loop.
...
The classe base for all items. Modding this affects every item in the game.
...
The base vehicle class. Mod it to change all vehicle comportamento.
...
Ecco un real-world pattern for a mod that enhances players, with opzionale support for two other mods:
...
## Pattern Professionali da Mod Reali

### Pattern 1: Non-Destructive Metodo Wrapping (COT Style)

Community Online Tools wraps methods by doing work before and after `super`, never replacing comportamento entirely:
...
VPP Admin Tools checks conditions before deciding whether to modify comportamento:
...
### Pattern 4: Funzionalita' Registration (Community Framework Style)
...
Unless you have a deliberate, well-understood reason to completely replace parent comportamento, always call `super`. Failing to do so breaks the mod chain and can crash servers.
...
In modded classes, you often work with objects that may not be initialized yet (perche' you are running before or after other code):
...
- Non swallow events silently (always call `super` unless deliberately overriding)
- Non overwrite fields that other mods might have set (add your own fields invece)
- Use `#ifdef` guards for opzionale dependencies
- Test with other popular mods loaded

### Rule 5: Use Descriptive Field Prefixes

When adding fields to a modded class, prefix them with your mod name to evita collisions with other mods adding fields to the same class:
...
This cannot be emphasized enough. Every time you vedi a bug report that says "Mod X broke when I added Mod Y," the first thing to check is whether someone forgot to call `super`.
...
### 2. Overriding a Metodo That Does Not Exist

Se try to `override` a method that does not exist in the classe genitore, you get a errore di compilazione. This usually happens when:
...
### 5. Forgetting `#ifdef` Guards for Opzionale Dependencies
...
4. Sempre calls `super` for normal (non-god-mode) damage
...
### Exercise 5: Opzionale Mod Integration
...
| Concetto | Details |
|---------|---------|
| Sintassi | `modded class ClassName { }` |
| Effect | Replaces the original class globally for all `new` calls |
| Chaining | Multiple mods can mod the same class; they chain in ordine di caricamento |
| `super` | **Sempre call it** unless deliberately replacing comportamento |
...
| Private accesso | Modded classes **can** accesso private members of the original |
| `#ifdef` guards | Usa per opzionale dependencies on other mods |
| Comune targets | `MissionServer`, `MissionGameplay`, `PlayerBase`, `ItemBase`, `DayZGame`, `CarScript` |
