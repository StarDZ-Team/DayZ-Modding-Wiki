...
Strings in Enforce Script are a **tipo valore**, like `int` or `float`. Sono passed by value and compared by value. The `string` type has a rich set of built-in methods for searching, slicing, converting, and formatting text. This chapter is a complete reference for every string operation available in DayZ scripting, with real-world examples from mod development.
...
## Complete String Metodo Reference

### Length

Restituisce the number of characters in the string.
...
Finds the first occurrence of a substring. Restituisce the index, or `-1` if not found.
...
Restituisce `true` if the string contains the given substring.
...
Builds a string using numbered placeholders `%1` through `%9`. Questo e' the principale way to build formatted strings in Enforce Script.
...
Placeholders are **1-indexed** (`%1` is the first argument, not `%0`). Puoi use up to 9 placeholders.
...
> **Nota:** Non c'e' `printf`-style formatting (`%d`, `%f`, `%s`). Only `%1` through `%9`.
...
C'e' also `Trim()` which returns a new trimmed string (available in some engine versions):
...
Non c'e' built-in case-insensitive comparison. Convert both strings to lowercase first:
...
| Treating strings as reference types | Strings are value types; assigning creates a copy | Questo e' usually fine -- just be aware that modifying a copy does not affect the original |
| Forgetting to create the array before `Split()` | Calling `Split()` on a null array causes a crash | Sempre: `TStringArray parts = new TStringArray;` before `Split()` |
