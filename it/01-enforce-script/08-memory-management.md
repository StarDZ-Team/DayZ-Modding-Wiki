...
[<< 1.7: Math & Vectors](07-math-vectors.md) | [Home](../../it/README.md) | [1.9: Casting & Reflection >>](09-casting-reflection.md)

---

## Introduzione

Enforce Script uses **automatic reference counting (ARC)** for memory management -- not garbage collection in the traditional sense. Understanding how `ref`, `autoptr`, and raw pointers work is essential for writing stable DayZ mods. Get it wrong and you will either leak memory (your server gradually consumes more RAM until it crashes) or accesso deleted objects (instant crash with no useful error message). Questo capitolo spiega every pointer type, when to use each, and how to evita the most dangerous pitfall: reference cycles.

---

## I Tre Tipi di Puntatore

Enforce Script has three ways to hold a reference to an object:

| Pointer Type | Keyword | Keeps Object Alive? | Zeroed on Delete? | Principale Use |
|-------------|---------|---------------------|-------------------|-------------|
| **Raw pointer** | *(none)* | No (riferimento debole) | Only if class extends `Managed` | Back-references, observers, caches |
...
A raw pointer is any variable declared without `ref` or `autoptr`. For class members, this creates a **riferimento debole**: it points to the object but does NOT keep it alive.
...
- **Managed classes** (most DayZ gameplay classes): When the object is deleted, all weak references are automatically set to `null`. Questo e' safe.
...
> **Rule:** Se are writing your own classes, always extend `Managed` for safety. Most DayZ engine classes (EntityAI, ItemBase, PlayerBase, etc.) already inherit from `Managed`.

---

## ref (Strong Reference)

The `ref` keyword marks a variable as a **riferimento forte**. The object stays alive as long as at least one riferimento forte exists. When the last riferimento forte is destroyed or overwritten, the object is deleted.
...
**Distinzione critica:** An `array<SafeZone>` holds **weak** references. An `array<ref SafeZone>` holds **strong** references. Se use the weak version, objects inserted into the array may be immediately deleted perche' no riferimento forte keeps them alive.
...
### Quando usare autoptr

In pratica, **local variables are already strong references by default** in Enforce Script. The `autoptr` keyword makes this explicit and self-documenting. Puoi use either:
...
> **Convenzione nel modding DayZ:** Most codebases use `ref` for class members and omit `autoptr` for locals (relying on the implicit riferimento forte comportamento). The CLAUDE.md for this project notes: "**`autoptr` is NOT used** -- use explicit `ref`." Follow whichever convention your project establishes.

---

## notnull Parameter Modifier

The `notnull` modifier on a parametro della funzione tells the compiler that null is not a valid argument. The compiler enforces this at call sites.
...
A reference cycle occurs when two objects hold strong references (`ref`) to each other. Neither object can ever be deleted perche' each one keeps the other alive. Questo e' the most common source of memory leaks in DayZ mods.
...
Break the cycle by making one side a riferimento debole. The "child" should hold a riferimento debole to its "parent":
...
Puoi manually delete an object at any time using `delete`. This destroys the object **immediately**, regardless of its reference count. All references (both strong and weak, on Managed classes) are set to null.
...
## Comportamento del Garbage Collection

Enforce Script does NOT have a traditional garbage collector that periodically scans for unreachable objects. Invece, it uses **deterministic reference counting:**

1. When a riferimento forte is created (assignment to `ref`, variabile locale, function argument), the object's reference count increases.
2. When a riferimento forte goes out of scope or is overwritten, the reference count decreases.
...
- Non ci sono "GC pauses" or unpredictable delays
...
## Real-World Esempio: Proper Manager Class

Ecco un complete example showing proper memory management patterns for a typical DayZ mod manager:
...
1. `s_Instance` is set to null, releasing the riferimento forte
...
5. `m_LastEditor` is a riferimento debole, nothing to clean up
...
| Not checking weak references for null | Crash when the referenced object has been deleted | Sempre: `if (weakRef) { weakRef.DoThing(); }` |
...
| Forgetting `ref` on membro della classe collections | Collezione is a riferimento debole, may be collected | Sempre: `protected ref array<...> m_List;` |
...
## Guida alla Scelta: Quale Tipo di Puntatore?
