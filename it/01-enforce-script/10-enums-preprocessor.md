...
> **Obiettivo:** Understand enum declarations, enum reflection tools, bitflag patterns, constants, and the preprocessor system for conditional compilation.
...
Se omit values, they auto-increment from the previous value (starting at 0):
...
> **Nota:** Enum inheritance is useful for extending vanilla enums in modded code without changing the original.
...
> **Attenzione:** Enforce Script does **not** validate enum assignments. Assigning an out-of-range integer to an enum variable compiles and runs without error.
...
Questo e' invaluable for logging and UI display:
...
Questo e' used when loading enum values from config files or JSON:
...
> **Nota:** Non c'e' `const` for reference types (objects). Non puoi make an object reference immutable.
...
> **Nota:** Enforce Script `#define` only creates existence flags. It does **not** support macro substitution (no `#define MAX_HP 100` — use `const` invece).

### Comune Engine Defines

DayZ provides these built-in defines based on build type and platform:

| Define | When Available | Use For |
|--------|---------------|---------|
| `SERVER` | Running on server dedicato | Server-only logic |
...
### Opzionale Mod Dependencies

Questo e' the standard pattern for mods that optionally integrate with other mods:
...
Se need multiple cases to share logic, use if/else:
