...
The absolute foundation. Code here runs at the engine level before any game systems exist. Questo e' the earliest point where mod code can execute.
...
The workhorse layer for configuration, data definitions, and systems that do not interact directly with world entities. Questo e' the first layer where game types are available.
...
**If in doubt, put it in `3_Game`.** Questo e' the default layer for most non-entity code. Configuration classes, enums, constants, RPC definitions, data classes -- all belong here.
...
Gameplay logic that interacts with the 3D world. This layer has accesso to entities, items, vehicles, buildings, and all world objects.
...
- Player extensions (modded `PlayerBase` comportamento)
...
The highest layer. Mission lifecycle, UI panels, HUD overlays, and the final initialization point. Questo e' where client-side and server-side startup code lives.
...
UI, HUD, menu screens, and mod initialization that depends on the mission being active. Anche the final place where the server hooks into startup/shutdown lifecycle.
...
Questo e' the single most important rule in DayZ script architecture. The engine enforces this at compile time.
...
### The Soluzione alternativa: Casting Through Base Types
...
## Ordine di Caricamento e Tempistica
...
**Importante:** Static variables and global-scope code in each layer execute during the compilation/linking phase, before `OnInit()` is ever called. Non put complex initialization logic in static initializers.
...
Questo e' the most common layer for mod code. Unless your code:
...
5. **None of the above?**
   Predefinito to `3_Game`.

### Keep Your Layers Thin

A common mistake is dumping everything into `4_World`. This creates tightly coupled code. Invece:
...
| Layer | Cartella | Voce di Configurazione | Principale Use | Frequenza |
|-------|--------|-------------|-------------|-----------|
| 1 | `1_Core/` | `engineScriptModule` | Constants, utilities, logging base | Raro |
| 2 | `2_GameLib/` | `gameLibScriptModule` | Engine bindings | Molto raro |
| 3 | `3_Game/` | `gameScriptModule` | Configs, RPCs, data classes | **Piu' comune** |
| 4 | `4_World/` | `worldScriptModule` | Entities, items, managers | Comune |
| 5 | `5_Mission/` | `missionScriptModule` | UI, HUD, mission hooks | Comune |

**Ricorda:** Lower layers cannot vedi higher layers. When in doubt, use `3_Game`. Move code up only when you need accesso to types defined in a higher layer.

---

**Successivo:** [Chapter 2.2: config.cpp Deep Dive](02-config-cpp.md)
