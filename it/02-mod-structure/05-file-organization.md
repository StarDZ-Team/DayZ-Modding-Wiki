...
> **Riepilogo:** How you organize files determines whether your mod is maintainable at 10 files or 1,000. Questo capitolo copre the canonical directory structure, naming conventions, content vs script vs framework mods, client-server splits, and lessons from professional DayZ mods.
...
## La Struttura delle Directory Canonica

Questo e' the standard layout used by professional DayZ mods. Not every folder is richiesto -- only create what you need.
...
Name each file after the principale class it contains:
...
- Scripts only when items need custom comportamento beyond what config defines
...
- Di solito depends on a framework (CF, DabsFramework, MyFramework)
...
## Mod con Divisione Client-Server

When a mod has both client-visible comportamento (UI, entity rendering) and server-only logic (spawning, AI brains, secure state), it should split into two packages.
...
4. **Mai put UI code in the server package** -- clients will not receive it
...
## PBO Naming and @mod Cartella Naming
...
The `@` has no technical meaning to the engine. E' purely organizational convention.
...
2. **Opzionale components** -- GUI PBO is opzionale if mod works headless
...
Each PBO has its own `config.cpp` with its own `CfgPatches` entry. The `requiredAddons` between them controls the ordine di caricamento:
...
Note: DabsFramework uses lowercase folder names (`scripts/`, `gui/`). This works perche' Windows is case-insensitive, but may cause issues on Linux. The convention is to use the canonical casing (`Scripts/`, `GUI/`).
...
**Soluzione:** One file per class, organized in subdirectories by subsystem.
...
**Soluzione:** Follow the layer rules from Chapter 2.1. Move entity code to `4_World` and UI code to `5_Mission`.
...
**Soluzione:** Sempre namespace with a subdirectory:
...
**Soluzione:** `stringtable.csv` goes at the mod root (next to `mod.cpp`):
...
**Soluzione:** Separate into multiple PBOs:
...
**Soluzione:** Keep nesting to 2-3 levels maximum. Flatten when possible:
...
**Soluzione:** Pick one convention and stick with it:
...
- [ ] Class names have a unique prefix to evita collisions
...
**Precedente:** [Chapter 2.4: Your First Mod -- Minimum Viable](04-minimum-viable-mod.md)
