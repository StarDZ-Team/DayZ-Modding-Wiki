...
Questo capitolo copre the P3D format structure, the LOD system, named selections, memory points, the proxy system, animation configuration via `model.cfg`, and the import workflow from standard 3D formats.
...
**P3D** (Point 3D) is Bohemia Interactive's binary 3D model format, inherited from the Real Virtuality engine and carried forward into Enfusion. E' a compiled, engine-ready format -- you do not write P3D files by hand.

### Key Caratteristiche

- **Binary format:** Not human-readable. Created and edited exclusively with Object Builder.
- **Multi-LOD container:** A single P3D file contains multiple LOD (Level of Dettaglio) meshes, each with a different purpose.
...
> **Importante:** When you pack a PBO with binarization enabled, your MLOD P3D files are automatically converted to ODOL. Se pack with `-packonly`, the MLOD files are included as-is. Entrambi work in-game, but ODOL is preferred for release builds.

---

## Object Builder

**Object Builder** is the Bohemia-provided tool for creating and editing P3D models. E' included in the DayZ Tools suite on Steam.
...
All paths in P3D files and materials are relative to the P: drive root. Per esempio, a material reference inside the model would be `MyMod\data\textures\my_item_co.paa`.
...
A P3D file contains multiple **LODs** (Levels of Dettaglio), each serving a specific purpose. The engine selects which LOD to use based on the situation -- distance from camera, physics calculations, shadow rendering, etc.

### LOD Types

| LOD | Resolution Value | Scopo |
|-----|-----------------|---------|
...
| Item Type | Obbligatorio LODs | Consigliato Additional LODs |
|-----------|---------------|----------------------------|
...
| Scopo | Esempio Selection Name | Used By |
|---------|----------------------|---------|
...
> **Suggerimento:** Selection names are case-sensitive. `Camo` and `camo` are different selections. Convention is lowercase.
...
Many effects need both a position and a direction. Questo e' achieved with paired memory points:
...
> **Nota:** The Memory LOD should contain ONLY named points (individual vertices). Non create faces or edges in the Memory LOD.

---

## Il Sistema Proxy

Proxies define positions where other P3D models can be attached. When you vedi a magazine inserted in a weapon, an optic mounted on a rail, or a suppressor screwed onto a barrel -- those are proxy-attached models.
...
**Sintomo:** Object has no collision. Players and bullets pass through it.
**Soluzione:** Create a Geometry LOD with a simplified convex mesh. Assign mass to vertices.

### 2. Non-Convex Collision Shapes

**Sintomo:** Physics glitches, objects bouncing erratically, items falling through surfaces.
**Soluzione:** Break complex shapes into multiple convex components in the Geometry LOD. Each component must be a closed convex solid.

### 3. Inconsistent Named Selections

**Sintomo:** Animations only work visually but not for collision, or shadow does not animate.
**Soluzione:** Assicurati every named selection that exists in the Resolution LOD also exists in Geometry, Fire Geometry, and Shadow LODs.

### 4. Wrong Scale

**Sintomo:** Object is gigantic or microscopic in-game.
**Soluzione:** Verifica your 3D software uses meters as the unit. A DayZ character is approximately 1.8 meters tall.

### 5. Missing Memory Points

**Sintomo:** Muzzle flash appears at the wrong position, attachments float in space.
**Soluzione:** Create the Memory LOD and add all richiesto named points at correct positions.

### 6. No Mass Defined

**Sintomo:** Object cannot be picked up, or physics interactions behave strangely.
**Soluzione:** Select all vertices in the Geometry LOD and assign mass via **Structure --> Mass**.
...
3. **Validate frequently.** Use Object Builder's **Structure --> Validate** after every significant change. Soluzione warnings before they become mysterious in-game bugs.
...
5. **Name selections descriptively.** Use `bolt_carrier` invece of `sel01`. Your future self (and other modders) will thank you.
