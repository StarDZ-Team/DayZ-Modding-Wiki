...
[<< Previous: 3D Models](02-models.md) | **Materials** | [Next: Audio >>](04-audio.md)
...
Questo capitolo copre the RVMAT file format, shader types, texture stage configuration, material properties, the damage-level material swap system, and practical examples drawn from DayZ-Samples.
...
An RVMAT file has a consistent structure. Ecco un complete, annotated example:
...
These are declared before the Stage classes and control the material's overall comportamento:

| Property | Type | Descrizione |
|----------|------|-------------|
| `ambient[]` | float[4] | Ambient light color multiplier. `{1,1,1,1}` = full, `{0,0,0,0}` = no ambient. |
| `diffuse[]` | float[4] | Diffuse light color multiplier. Di solito `{1,1,1,1}`. |
| `forcedDiffuse[]` | float[4] | Additive override to diffuse. Di solito `{0,0,0,0}`. |
...
The `PixelShaderID` and `VertexShaderID` values determine which rendering pipeline processes the material. Entrambi should usually be set to the same value.

### Available Shaders

| Shader | Caso d'Uso | Texture Stages Obbligatorio |
|--------|----------|------------------------|
...
Se are creating a mod item (weapon, clothing, tool, container), you will almost always use the Super shader.
...
| Stage | Texture Role | Typical Suffix | Descrizione |
|-------|-------------|----------------|-------------|
...
| **Stage4** | Ambient Shadow | `_as` | Pre-baked ambient occlusion (opzionale) |
| **Stage5** | Macro map | `_mc` | Large-scale color variation (opzionale) |
| **Stage6** | Dettaglio map | `_de` | Tiling micro-detail (opzionale) |
| **Stage7** | Emissive / Light map | `_li` | Self-illumination (opzionale) |
...
Questo e' commonly used for terrain materials and building surfaces where the same detail texture repeats.
...
Questo e' not a top-level RVMAT property but is configured in config.cpp or through the material's shader settings depending on the use case.
...
> **Suggerimento:** Non devi always need unique textures for every damage level. A common optimization is to share the normal and specular maps across all levels and only change the diffuse texture:
>
> ```
...
## Come i Materiali Referenziano le Texture
...
**Sintomo:** Texture appears scrambled, normal map shows as color, color shows as bumps.
**Soluzione:** Assicurati Stage1 = normal, Stage2 = diffuse, Stage3 = specular (for the Super shader).

### 2. Misspelling `emmisive`

**Sintomo:** Emissive does not work.
**Soluzione:** Bohemia uses `emmisive` (double m, single s). Using the correct English spelling `emissive` will not work. Questo e' a known historical quirk.

### 3. Texture Path Mismatch

**Sintomo:** Model appears with default gray or magenta material.
**Soluzione:** Verifica that texture paths in the RVMAT exactly match the file locations relative to P: drive. Paths use backslashes. Controlla capitalization -- some systems are case-sensitive.

### 4. Missing RVMAT Assignment in P3D

**Sintomo:** Model renders with no material (flat gray or default shader).
**Soluzione:** Open the model in Object Builder, select faces, and assign the RVMAT via **Face Properties**.

### 5. Using Wrong Shader for Transparent Items

**Sintomo:** Transparent texture appears opaque, or entire surface vanishes.
**Soluzione:** Use `Glass`, `AlphaTest`, or `AlphaBlend` shader invece of `Super` for transparent surfaces. Use `_ca` suffix textures with proper alpha channels.
...
6. **Document your material settings.** When you find specular/power values that work well for a surface type, record them. Dovrai reuse these settings across many items.
