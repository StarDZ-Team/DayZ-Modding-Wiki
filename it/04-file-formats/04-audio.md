...
### OGG Vorbis (Principale Format)

**OGG Vorbis** is DayZ's principale audio format. All custom sounds should be exported as `.ogg` files.
...
| **Quality range** | -1 to 10 (5-7 consigliato for game audio) |

### Key Rules for OGG in DayZ

- **3D positional sounds MUST be mono.** Se provide a stereo file for a 3D sound, the engine may not spatialize it correctly or may ignore one channel.
...
|----------|-------|
| **Extension** | `.wss` |
| **Status** | Legacy, not consigliato for new mods |
| **Conversion** | WSS files can be converted to OGG with Audacity or similar tools |

Dovrai encounter WSS files when examining vanilla DayZ data or porting content from older Bohemia games.
...
The `rangeCurve[]` in a SoundShader defines how volume decreases with distance. E' an array of `{distance, volume}` pairs:
...
The engine interpolates linearly between defined points. Puoi create any falloff curve by adding more control points.

### Predefined Volume Curves

SoundSets can reference named curves via the `volumeCurve` property:

| Curve Name | Comportamento |
|------------|----------|
...
Looping sounds repeat continuously until explicitly stopped. Sono used for engines, ambient atmosphere, alarms, and any sustained audio.
...
1. **Zero-crossing at start and end.** The waveform should cross zero amplitude at both endpoints to evita a click/pop at the loop point.
...
## Aggiungere Suoni Personalizzati a un Mod
...
Audacity is the consigliato tool for DayZ audio production:
...
**Sintomo:** Sound does not spatialize, plays centered or only in one ear.
**Soluzione:** Convert to mono before exporting. 3D positional sounds require mono audio files.

### 2. File Extension in samples[] Path

**Sintomo:** Sound does not play, no error in log (engine silently fails to find the file).
**Soluzione:** Remove the `.ogg` extension from the path in `samples[]`. The engine adds it automatically.
...
**Sintomo:** SoundShaders or SoundSets not recognized, sounds do not play.
**Soluzione:** Add `"DZ_Sounds_Effects"` to your CfgPatches `requiredAddons[]` to ensure the base sound system loads before your definitions.

### 4. Range Too Short

**Sintomo:** Sound cuts off abruptly at a short distance, feels unnatural.
**Soluzione:** Set `range` to a realistic value. Gunshots should carry 300-800m, footsteps 20-40m, voices 50-100m.

### 5. No Random Variation

**Sintomo:** Sound feels repetitive and artificial after hearing it multiple times.
**Soluzione:** Provide multiple samples in the SoundShader and add `frequencyRandomizer` to the SoundSet for pitch variation.
...
**Sintomo:** Sound crackles or distorts, especially at close range.
**Soluzione:** Normalize your audio to -1 dB or -3 dB peak in Audacity before exporting. Mai set `volume` or `volumeFactor` above 1.0 unless the source audio is very quiet.

---

## Buone Pratiche

1. **Sempre export 3D sounds as mono OGG.** Questo e' the single most important rule. Stereo files will not spatialize.
