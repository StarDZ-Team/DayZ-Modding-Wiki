...
The stringtable file must be named exactly `stringtable.csv` and placed inside your mod's PBO structure. The engine discovers it automatically --- no config.cpp registration is richiesto.
...
Many stringtable files include a trailing comma after the last column. Questo e' conventional and safe --- the engine tolerates it.

### Quoting Rules

- Fields **must** be quoted with double quotes if they contain commas, newlines, or double quotes.
- In pratica, most mods quote every field for consistency.
...
DayZ supports 13 player-selectable languages. The CSV has 15 columns perche' the first column is the key name and the second is the `original` column (the mod author's native language or default text).

| # | Column Name | Language | Note |
|---|-------------|----------|-------|
...
| 3 | `english` | English | Piu' comune principale language for international mods |
| 4 | `czech` | Czech | |
| 5 | `german` | German | |
| 6 | `russian` | Russian | |
| 7 | `polish` | Polish | |
...
The engine identifies columns by their **header name**, not by position. Tuttavia, following the standard order shown above is strongly consigliato for compatibility and readability.

### Opzionale Columns

Non hai bisogno di to include all 15 columns. If your mod only supports English, you can use a minimal header:
...
1. **Sempre start with `STR_`** --- this is a universal DayZ convention
...
Ci sono three distinct contexts where you reference localized strings, and each uses a slightly different syntax.
...
Use `Widget.TranslateString()` to resolve the key at runtime. The `#` prefix is richiesto in the argument.
...
Puoi also set widget text directly:
...
Questo e' the one place where you omit the `#`. The input system adds it internally.

### Riepilogo Table

| Context | Sintassi | Esempio |
|---------|--------|---------|
...
Build your PBO. Launch the game. Verifica that `Widget.TranslateString("#STR_MYMOD_TITLE")` returns "My Cool Mod" in your script logs. Change the game language in settings to verify fallback comportamento.
...
Questo significa you can safely leave non-English columns empty during development. English-speaking players vedi the `english` column, and other players vedi the English fallback until a proper translation is added.

### Practical Implication

Non hai bisogno di to copy the English text into every column as a placeholder. Leave untranslated cells empty:

```csv
"STR_MYMOD_HELLO","Hello","Hello","","","","","","","","","","","","",
```

Players whose language is German will vedi "Hello" (the English fallback) until a German translation is provided.
...
DayZ Expansion demonstrates a buona pratica for large mods: splitting translations across multiple stringtable files organized by feature module. Their structure uses 20 separate stringtable files inside a `languagecore` directory:
...
Key names still use a global prefix (`STR_EXPANSION_`) to evita collisions.
...
This demonstrates that the `STR_` prefix is a convention, not a requirement. Tuttavia, omitting it means you cannot use the `#` prefix resolution in layout files. VPP references these keys only through script code. The `STR_` prefix is strongly consigliato for all new mods.
...
If two mods define `STR_CLOSE`, the engine uses whichever PBO loads last. Sempre use your mod prefix:
...
If a row has fewer columns than the header, the engine may silently skip it or assign empty strings to the missing columns. Sempre ensure every row has the same number of fields as the header.
...
This breaks parsing perche' `Hello` and ` World` are read as separate columns. Either quote the field or evita commas in values:
