...
- **Object** adds position/orientation helpers, health, config accesso, hidden selections, and type checking (`IsMan()`, `IsBuilding()`, etc.).
...
The engine-native entity. All proto native methods --- you cannot vedi their implementation in script.
...
| `GetSibling` | `proto native IEntity GetSibling()` | Successivo sibling entity |
...
Base class for all spatial objects in the game world. Questo e' the first script-accessible level of the hierarchy --- `IEntity` is purely engine-native.
...
**Esempio --- set an item to half health:**
...
### Config Accesso (on the entity itself)
...
Comune inventory operations through the returned `GameInventory`:
...
**Esempio --- custom item with action:**
...
**Esempio --- get player info on server:**
...
Animals use the same health, position, and damage APIs as other entities. Their comportamento is driven by the AI system and CE-configured territory files.
...
Questo e' the preferred API. The `iFlags` parameter uses ECE (Entity Creation Event) flags.

### ECE Flags

| Flag | Value | Descrizione |
|------|-------|-------------|
| `ECE_NONE` | `0` | No special comportamento |
...
| `ECE_NOSURFACEALIGN` | `262144` | Non align to surface normal |
...
Deferred deletion --- the object is removed on the next frame via `CallQueue`. Questo e' the safest way to delete objects perche' it avoids issues with deleting objects while they are being iterated.
...
**Esempio --- cleanup spawned objects:**
...
|---------|------------|
| Gerarchia | `IEntity` > `Object` > `EntityAI` > `ItemBase` / `PlayerBase` / `ZombieBase` |
| Posizione | `GetPosition()` / `SetPosition()` disponibili da `Object` in su |
| Salute | Basata su zone: `GetHealth(zone, type)` / `SetHealth(zone, type, value)` |
| IsAlive | Usa su `EntityAI` o prima fai il cast: `EntityAI eai; Class.CastTo(eai, obj)` |
| Inventario | `eai.GetInventory()` restituisce `GameInventory` con CRUD completo |
| Creazione | `GetGame().CreateObjectEx(type, pos, ECE_flags)` e l'API preferita |
| Eliminazione | `obj.Delete()` (differita) o `GetGame().ObjectDelete(obj)` (immediata) |
| Sincronizzazione rete | `RegisterNetSyncVariable*()` nel costruttore, reagisci in `OnVariablesSynchronized()` |
| Controllo tipo | `obj.IsKindOf("ClassName")`, `obj.IsMan()`, `obj.IsBuilding()` |

---

[Home](../../it/README.md) | **Entity System** | [Next: Vehicles >>](02-vehicles.md)
