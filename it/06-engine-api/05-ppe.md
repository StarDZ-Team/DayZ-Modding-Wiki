...
The `PPEManager` is a singleton that coordinates all active PPE requests. You rarely interact with it directly --- invece, you work through `PPERequester` subclasses.
...
A static registry that holds instances of all PPE requesters. Accesso specific requesters by their constant index.
...
## Comune PPE Material IDs

Effects target specific post-processing materials. Comune material IDs:
...
Registration is handled by adding the requester to the bank. In pratica, most modders use the built-in requesters and modify their parameters rather than creating fully custom ones.
...
|---------|-----------|
| Accesso | `PPERequesterBank.GetRequester(CONSTANT)` |
...
| Comune effects | Blur, vignette, saturation, NVG, flashbang, grain, chromatic aberration |
