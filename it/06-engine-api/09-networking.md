...
The principale RPC class for sending custom data between client and server. `ScriptRPC` extends `ParamsWriteContext`, so you call `.Write()` on it directly to serialize data.
...
RPCs are received by overriding `OnRPC` on the target object (or any classe genitore in the hierarchy).
...
Restituisce `true` on success, `false` if the read fails (wrong type, insufficient data). Sempre check the return value.
...
**Esempio --- full client-server exchange:**
...
> **Nota:** `ScriptInputUserData` has rate limiting. Sempre check `CanStoreInputUserData()` before sending.
...
For simple state synchronization, `RegisterNetSyncVariable*()` is often simpler than RPCs. Vedi [Chapter 6.1](01-entity-system.md) for details.
...
**Mai trust client data.** Sempre validate RPC data on the server:
...
- Using net sync variables invece
...
|---------|-----------|
| ScriptRPC | Principale RPC class: `Write()` data, then `Send(target, id, guaranteed, recipient)` |
| OnRPC | Override on target object to receive: `OnRPC(sender, rpc_type, ctx)` |
| Leggi/Write | `ctx.Write(value)` / `ctx.Read(value)` --- always check Leggi return value |
...
| IDs | Use high numbers (87000+) to evita vanilla conflicts |
| Security | Sempre validate client data on the server |
