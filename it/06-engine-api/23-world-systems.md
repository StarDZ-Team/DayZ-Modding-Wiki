...
Questo capitolo tratta five mission-folder configuration systems:
...
Contaminated areas are toxic gas zones that damage players without protective equipment. They are configured through `cfgEffectArea.json` in the mission del tuo mod.

### Key Concepts

- **Static areas** are definiscid in the JSON file and created at mission start. They are **not persistent** --- you can add or remove zones between restarts with no wipe required.
...
- To **disable all effect areas**, place an empty JSON file (`{}`) in the mission del tuo mod.
...
Configuration is stored in `cfgundergroundtriggers.json` in the mission del tuo mod. For a working example, see the [official DayZ Central Economy repository](https://github.com/BohemiaInteractive/DayZ-Central-Economy).

> **Nota:** The JSON snippets below include comments for clarity. Real JSON files must not contain comments.

### Configuration Objects

The file definiscis two types of objects:
...
| Tipo | Breadcrumbs? | EyeAccommodation | Scopo |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | Costante darkness deep underground. Eye accommodation is fixed at the configured value |
...
| `Radius` | float | Influence radius in meters. Set to `-1` for il motore default |
...
> **Suggerimento:** When using `UseRaycast: 1`, raise the breadcrumb position slightly off the floor (a few centimeters in the Y axis) to avoid the ray being blocked by the ground surface.
...
| Attribute | Tipo | Predefinito | Descrizione |
|-----------|------|---------|-------------|
...
| Element | Attributes | Descrizione |
|---------|------------|-------------|
...
> **Nota:** You only need to include the phenomena you want to override. Omitted phenomena use engine defaults.
...
| Tipo | Parametro | Predefinito | Descrizione |
|------|-----------|---------|-------------|
...
| Parametro | Predefinito | What It Disables |
|-----------|---------|------------------|
...
| Tipo | Parametro | Predefinito | Descrizione |
|------|-----------|---------|-------------|
| bool | `hitDirectionSovrascriviEnabled` | `false` | Enable custom hit indicator settings |
...
Esempio: `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
| Setting `staminaMax` to 0 | Undefiniscid behavior, potential crash | Use a positive value (minimum 1.0) |
...
- **Sovrascrivis the script state machine.** If both `cfgweather.xml` and a scripted `WeatherOnBeforeChange()` override are active, the XML limits apply and the script operates within those limits.
- **Only one cfgweather.xml is loaded** per mission del tuo mod. Multiple weather mods cannot stack XML files.

### Gameplay Settings

- **One cfgGameplay.json per mission.** Values are global --- they affect all players on the server.
- **The file version must match the game version.** Outdated files may have missing parameters that default to engine valori.
- **Mod interactions:** Mods that override stamina, base building, or navigation systems in script may conflict with or override cfgGameplay.json valori.
