...
[<< Previous: Singleton Pattern](01-singletons.md) | [Home](../../it/README.md) | [Next: RPC Patterns >>](03-rpc-patterns.md)
...
Community Framework (CF) provides the most widely-used module system in the DayZ modding ecosystem. Entrambi COT and Expansion build on it.
...
Invece of manually registering modules, you annotate a class with an attribute, and the framework discovers it at startup using reflection:
...
MyFramework uses an explicit registration pattern with a static manager class. Non c'e' instance of the manager --- it is entirely static methods and static storage.
...
1. **OnInit comes before OnMissionStart.** Mai load configs or spawn entities in `OnInit()` --- the world may not be ready yet.
2. **OnUpdate receives delta time.** Sempre use `dt` for time-based logic, never assume a fixed frame rate.
3. **OnMissionFinish must clean up everything.** Every `ref` collection must be cleared. Every event subscription must be removed. Every singleton must be destroyed. Questo e' the only reliable teardown point.
4. **Modules should not depend on each other's initialization order.** If Module A needs Module B, use lazy accesso (`GetModule()`) rather than assuming B was registered first.

---

## Buone Pratiche for Module Design

### 1. One Module, One Responsibility

A module should own exactly one domain. Se find yourself writing `VehicleAndWeatherAndLootModule`, split it.
...
### 4. Use the Module Manager for Cross-Module Accesso

Non hold direct references to other modules. Use the manager's lookup:
...
| Funzionalita' | CF_ModuleCore | VPP Plugin | Dabs Attribute | MyMod Module |
|---------|--------------|------------|----------------|---------------|
...
| **Cross-mod accesso** | `CF_Modules<T>.Get()` | `GetPluginManager().Get()` | `CF_Modules<T>.Get()` | `MyModuleManager.GetModule()` |

Choose the approach that matches your mod's dependency profile. Se already depend on CF, use `CF_ModuleCore`. Se want zero external dependencies, build your own system following the MyMod or VPP pattern.
