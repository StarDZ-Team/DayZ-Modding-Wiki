...
Questo capitolo copre the fundamental `ScriptRPC` pattern, the client-server roundtrip lifecycle, error handling, and then compares the three major RPC routing approaches used in the DayZ modding community.
...
### Complete Esempio: Teleport Request
...
Every server-side RPC handler that performs a privileged action **must** check permissions before executing. Mai trust the client.
...
Sempre log failed permission checks. This creates an audit trail and helps server owners detect exploit attempts:
...
Every `ctx.Read()` can fail. Sempre check the return value:
...
For events that all clients should vedi (killfeed, announcements, weather changes), the server broadcasts with `identity = null`:
...
## Serialization: The Leggi/Write Contract

The single most important rule of DayZ RPCs: **the Leggi order must exactly match the Write order, type for type.**
...
Se swap the read order, the deserializer interprets bytes intended for one type as another. An `int` read where a `string` was written will produce garbage, and every subsequent read will be offset --- corrupting all remaining fields. The engine does not throw an exception; it silently returns wrong data or causes `Read()` to return `false`.
...
Non c'e' built-in array serialization. Write the count first, then each element:
...
For complex data, serialize field by field. Non try to pass objects directly through `Write()`:
...
| Funzionalita' | CF Named | Integer-Range | MyMod String-Routed |
|---------|----------|---------------|---------------------|
...
| **Payload style** | Param wrappers | Raw Write/Leggi | Raw Write/Leggi |
...
**Soluzione:** Write a comment block documenting the field order at both the send and receive sites:
...
The server cannot read client-side widget state, input state, or local variables. Se need to send a UI selection to the server, serialize the relevant value (a string, an index, an ID) --- not the widget object itself.
...
**Soluzione:** Sempre unregister or clean up handlers on mission finish:
...
1. **Sempre check `ctx.Read()` return values.** Every read can fail. Return immediately on failure.

2. **Sempre validate the sender on the server.** Controlla that `sender` is non-null and has the richiesto permission before doing anything.
...
8. **Use `CreateRPC()` for payloads, `Send()` for signals.** Se have no data to send (just a "do it" signal), use the header-only `Send()`. Se have data, use `CreateRPC()` + manual writes + manual `rpc.Send()`.

---

[<< Previous: Module Systems](02-module-systems.md) | [Home](../../it/README.md) | [Next: Config Persistence >>](04-config-persistence.md)
