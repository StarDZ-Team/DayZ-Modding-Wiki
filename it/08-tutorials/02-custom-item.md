...
> **Riepilogo:** This tutorial walks you through adding a brand-new item to DayZ. Dovrai define the item in config.cpp, give it textures using hidden selections, add it to the server's spawn table, create a string table for its display name, and test it in-game. By the end, you will have a custom item that players can find, pick up, and carry in their inventory.
...
Noi create an item called **Field Journal** -- a small notebook that players can find in the world, pick up, and store in their inventory. It will:
...
Questo e' the standard workflow for creating any item in DayZ, whether it is food, tools, clothing, or building materials.
...
- A text editor
- DayZ Tools installed (for texture conversion, opzionale)

Noi build on top of the mod from Chapter 8.1. Your current structure should look like:
...
E' buona pratica to keep item definitions in a separate PBO from your scripts. Create a new folder structure:
...
The first entry in `hiddenSelectionsTextures` replaces the first entry in `hiddenSelections`. Se had multiple selections:
...
For initial testing, you can point `hiddenSelectionsTextures` to a vanilla texture invece of creating your own:
...
1. **Create a source image:**
   ...
   - Create a new image at **512x512 pixels** (power-of-2 dimensions are richiesto: 256, 512, 1024, 2048)
...
| `_de` | Dettaglio | Additional detail overlay |
...
- Tutti i valori must be double-quoted
...
**Option A: Separate PBO (Consigliato)**
...
2. **Does it have the right name?** Controlla that "Field Journal" appears (not `$STR_MFM_FieldJournal`). Se vedi the raw string reference, the stringtable is not loading.
...
Questo e' a significant undertaking. For most items, retexturing a vanilla model (as we did above) is sufficient.
...
### Adding Script Comportamento

To give your item custom comportamento (for example, an action when the player uses it), create a script class in `4_World`:
...
### types.xml Entry (Server Mission Cartella)
...
- **config.cpp not loaded:** Controlla that your Data PBO is packed and loaded, or that file patching is active.
...
- **Stringtable not found:** Assicurati `Stringtable.csv` is in the same PBO as the config that references it, or in the mod root.
...
- **Texture path wrong:** Verifica that `hiddenSelectionsTextures[]` points to the correct `.paa` file path. Paths use backslashes in config.cpp.
- **Hidden selection name wrong:** The selection name must match what the model defines. Controlla using Object Builder.
- **Texture not in PBO:** If using packed PBOs, the texture file must be inside the PBO.

### Item Non puo' Be Picked Up

- **`scope` not set to 2:** Assicurati `scope = 2;` is in your item class.
...
4. **Create clothing** -- Extend `Clothing_Base` invece of `Inventory_Base` for wearable items.
5. **Build a weapon** -- Extend `Weapon_Base` for firearms with attachments and animations.

---

**Precedente:** [Chapter 8.1: Your First Mod (Hello World)](01-first-mod.md)
**Successivo:** [Chapter 8.3: Building an Admin Panel Module](03-admin-panel.md)
