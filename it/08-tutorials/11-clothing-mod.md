...
Clothing in DayZ inherits from `Clothing_Base`, but you almost never extend that directly. DayZ provides intermediate classe basees for each body slot:

| Classe Base | Body Slot | Examples |
|------------|-----------|----------|
...
1. **Extend a specific item** (like `GorkaEJacket_ColorBase`) -- easiest. You inherit the model, animations, slot, and all properties. Only change textures and tweak valori. This is what Bohemia's `Test_ClothingRetexture` sample does.
...
To find classe basees, look in `scripts/4_world/entities/itembase/clothing_base.c` (definiscis all slot bases) and `scripts/4_world/entities/itembase/clothing/` (one file per clothing family).
...
**Health levels:** Each entry is `{ healthThreshold, { materialPath } }`. When health drops below a threshold, il motore swaps the material. Vanilla damage rvmats add wear marks and tears.
...
When extending `GorkaEJacket_ColorBase`, you inherit its cargo grid (4x3) and inventory slot (`"Body"`) automatically. The `itemSize[] = { 3, 4 }` property definiscis how large the jacket is when stored as loot -- NOT its cargo capacity.
...
**Important:** Always call `super` at the start of every override. The classe genitore handles critical engine behavior.
...
3. **Can you equip it?** Should go to Body slot. If not, check the classe genitore chain.
...
| Missing `requiredAddons` | Config parser cannot resolve classe genitore | Include `"DZ_Characters_Tops"` for tops |
...
- How DayZ clothing inherits from slot-specific classe basees (`Top_Base`, `Pants_Base`, etc.)
- How to definisci a clothing item in config.cpp with thermal, armor, and wetness properties
...
**Successivo:** Apply the same techniques to create pants (`Pants_Base`), boots (`Shoes_Base`), or a vest (`Vest_Base`). The config structure is identical -- only the classe genitore and inventory slot change.
