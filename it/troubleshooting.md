...
Questi sono problemi in cui il mod non appare, non si attiva o viene rifiutato dal gioco all'avvio.

| Sintomo | Causa | Soluzione |
|---------|-------|-----|
| "Addon requires addon X" error at startup | Missing or incorrect `requiredAddons[]` entry | Add the exact `CfgPatches` class name of the dependency to your `requiredAddons[]`. Names are case-sensitive. Vedi [Capitolo 2.2](02-mod-structure/02-config-cpp.md). |
| Mod not visible in launcher | `mod.cpp` file is missing or has syntax errors | Create or fix `mod.cpp` in your mod root. It must contain `name`, `author`, and `dir` fields. Vedi [Capitolo 2.3](02-mod-structure/03-mod-cpp.md). |
| "Config parse error" on startup | Sintassi error in `config.cpp` | Check for missing semicolons after class closings (`};`), unclosed braces, or unbalanced quotes. Every class body ends with `};`, every property ends with `;`. |
...
| Mod loads but nothing happens | Scripts compile but never execute | Check that your mod has an entry point: a `modded class MissionServer` or `MissionGameplay`, a registered module, or a plugin. Scripts do not run by themselves. Vedi [Capitolo 7.2](07-patterns/02-module-systems.md). |
...
| Mod loads only in singleplayer | Server does not have the mod installed | Ensure the server's `-mod=` launch parameter includes your mod path, and the PBO is in the server's `@YourMod/Addons/` del tuo mod. |
| "Addon X is not signed" | Server requires signed addons | Sign your PBOs with your private key and provide the `.bikey` to the server's `keys/` del tuo mod. Vedi [Capitolo 4.6](04-file-formats/06-pbo-packing.md). |

---

## 2. Errori di Script

Questi appaiono nel log degli script come `SCRIPT (E):` or `SCRIPT ERROR:` lines.

| Sintomo | Causa | Soluzione |
|---------|-------|-----|
| `Null pointer access` | Accessing a variable that is `null` | Add a null check before using the variable: `if (myVar) { myVar.DoSomething(); }`. This is the most common errore a runtime. |
| `Cannot convert type 'X' to type 'Y'` | Direct cast between incompatible types | Use `Class.CastTo()` for safe downcasting: `Class.CastTo(result, source);`. Never assume a cast will succeed. Vedi [Capitolo 1.9](01-enforce-script/09-casting-reflection.md). |
| `Undefiniscid variable 'X'` | Typo, wrong scope, or wrong layer | Check spelling first. If the variable is a class from another file, ensure it is definiscid in the same or lower layer. `3_Game` cannot see `4_World` types. Vedi [Capitolo 2.1](02-mod-structure/01-five-layers.md). |
| `Method 'X' not found` | Calling a method that does not exist on that class | Verify the method name and check the classe genitore. You may need to cast to a more specific type first. Check vanilla scripts at `P:\DZ\scripts\` for the correct API. |
...
| `Redeclaration of variable 'X'` | Same variable name declared in sibling `else if` blocks | Dichiara la variabile una volta prima della `if`/`else` chain, then assign inside each branch. Vedi [Capitolo 1.12](01-enforce-script/12-gotchas.md). |
| `Member already definiscid` | Duplicate variable or method name in a class | Check for copy/paste errori. Each member name deve essere unico within a class hierarchy (including classe genitorees). |
...
| Sintassi error with no clear message | Backslash `\` or escaped quote `\"` in string literal | Enforce Script's CParser non supporta `\\` or `\"`. Use forward slashes for paths (`"my/path/file"`). For quotes, use single-quote characters. Vedi [Capitolo 1.12](01-enforce-script/12-gotchas.md). |
| `JsonFileLoader` returns null data | Assigning the return value of `JsonLoadFile()` | `JsonLoadFile()` returns `void`. Pre-allocate the object and pass it by reference: `ref MyConfig cfg = new MyConfig(); JsonFileLoader<MyConfig>.JsonLoadFile(path, cfg);`. Vedi [Capitolo 6.8](06-engine-api/08-file-io.md). |
...
| No ternary operator support | Using `condition ? a : b` syntax | Enforce Script has no ternary operator. Use an `if`/`else` block instead. Vedi [Capitolo 1.12](01-enforce-script/12-gotchas.md). |
| `do...while` loop error | Using `do { } while(cond)` | Enforce Script non supporta `do...while`. Use a `while` loop with a `break` condition instead. Vedi [Capitolo 1.12](01-enforce-script/12-gotchas.md). |
...
Problemi con le Remote Procedure Call e la comunicazione client-server.

| Sintomo | Causa | Soluzione |
|---------|-------|-----|
| RPC sent but never received | Registration mismatch | Both sender and receiver must register the same RPC ID. Verify the ID matches exactly on both client and server. Vedi [Capitolo 6.9](06-engine-api/09-networking.md). |
...
| Data not syncing to clients | Missing `SetSynchDirty()` | After changing any variable registered for synchronization, call `SetSynchDirty()` on the entity. Without it, il motore does not broadcast changes. |
...
| RPC floods and server lag | Sending RPCs ogni frame or in tight loops | Throttle RPC calls with timers or accumulators. Batch multiple small updates into a single RPC. Use Net Sync Variables for data that changes frequently. |
...
| `OnRPC()` never called | Sovrascrivi is in the wrong class or layer | `OnRPC()` must be overridden on the entity that receives the RPC. If overriding on `PlayerBase`, ensure you call `super.OnRPC()` so other mods still work. |
| Net Sync Variables not updating on client | Missing `RegisterNetSyncVariable*()` or wrong type | Register each variable in the constructor with the correct method (`RegisterNetSyncVariableInt`, `RegisterNetSyncVariableFloat`, `RegisterNetSyncVariableBool`). Sovrascrivi `OnVariablesSynchronized()` to react to changes on the client side. |
| RPC works for host but not other players | Using player object riferimento instead of identity | On dedicated servers, the host player is not special. Ensure you are using `PlayerIdentity` for targeting and not relying on local player references that only exist on the sender's machine. |

---

## 4. Problemi dell'Interfaccia

Problemi con layout GUI, widget, menu e input.

| Sintomo | Causa | Soluzione |
|---------|-------|-----|
| Layout loads but nothing is visible | Widget size is zero | Check `hexactsize` and `vexactsize` valori. Both must be greater than zero. Do not use negative sizes. Vedi [Capitolo 3.3](03-gui-system/03-sizing-positioning.md). |
...
| Text shows `#STR_some_key` literally | Stringtable entry is missing or file is not loaded | Add the key to your `stringtable.csv`. Check that the CSV is in your mod root and has the correct `Language,Key,Original` header format. Vedi [Capitolo 5.2](05-config-files/02-inputs-xml.md). |
...
| ScrollWidget content does not scroll | Content is not inside a WrapSpacer or child widget | ScrollWidget needs a single child (usually a `WrapSpacer` or `FrameWidget`) that is larger than the scroll area. Put your content widgets inside that child. Vedi [Capitolo 3.3](03-gui-system/03-sizing-positioning.md). |
...
| UI looks different at other resolutions | Using hardcoded pixel values | Use proportional sizing (`halign`, `valign`, `hfill`, `vfill`) instead of fixed pixel valori. Test at multiple resolutions. Vedi [Capitolo 3.3](03-gui-system/03-sizing-positioning.md). |

---

## 5. Problemi di Build e PBO

Problemi con l'impacchettamento, la binarizzazione e il deploy dei mod.
...
| "Signature check failed" on server connect | PBO is unsigned or signed with wrong key | Re-sign the PBO with your private key using DSSignFile. Ensure the server has the matching `.bikey` in its `keys/` del tuo mod. |
...
| "Prefix mismatch" warning | PBO prefix does not match `config.cpp` | Ensure the `$PBOPREFIX$` file content matches the addon prefix definiscid in your `config.cpp` `CfgPatches`. |
...
| Old version of mod loads despite changes | Cached PBO or workshop version overriding | Delete the old PBO. Check that the game is not loading a cached workshop version. Verify the `-mod=` path points to your development folder, not the workshop del tuo mod. |
| Addon Builder reports "no entry" warnings | Config references a property that does not exist | These warnings are usually non-fatal. Check that all `CfgVehicles` classe basees exist. Missing entries in inherited configs cause cascading warnings. |
| PBO packing includes unwanted files | No `.pboignore` or filter set | Addon Builder packs everything in the source del tuo mod. Use a `.pboignore` file or explicitly exclude file types (`.psd`, `.blend`, `.bak`) in the builder settings. |

---

## 6. Problemi di Prestazioni

Calo degli FPS del server o del client, problemi di memoria e operazioni lente.

| Sintomo | Causa | Soluzione |
|---------|-------|-----|
| Low server FPS (below 20) | Heavy processing in `OnUpdate()` or per-frame methods | Use a delta-time accumulator to throttle work: only execute logica every N seconds. Move expensive operations to timers or scheduled callbacks. Vedi [Capitolo 7.7](07-patterns/07-performance.md). |
| Memory grows over time (perdita di memoria) | `ref` reference cycles preventing garbage collection | When two objects hold `ref` references to each other, neither is ever freed. Make one side a raw (non-`ref`) reference. Break cycles in cleanup methods. Vedi [Capitolo 1.8](01-enforce-script/08-memory-management.md). |
...
| Network lag spikes | Too many RPCs or large RPC payloads | Batch small updates into fewer RPCs. Use Net Sync Variables for frequently changing valori. Compress data where possible. |
...
Problemi con oggetti personalizzati, veicoli ed entita del mondo.
...
| Attachments do not snap to item | Wrong `inventorySlot` names | Attachment slot names deve corrispondere esattamente between the parent item's `attachments[]` and the child item's `inventorySlot[]`. Names are case-sensitive. |
| Item damage zones not working | `DamageSystem` config mismatch with model | Each `DamageZone` name must match a named selection in the model's Fire Geometry LOD. Check with Object Builder. Vedi [Capitolo 6.1](06-engine-api/01-entity-system.md). |
...
| Item has wrong weight or size | `weight` and `itemSize[]` config values | `weight` is in grams (integer). `itemSize[]` definiscis the inventory grid slots as `{width, height}`. Check classe genitore values if inheriting. |
...
| `types.xml` changes ignored | Editing the wrong `types.xml` file | The server loads types from `mpmissions/your_mission/db/types.xml`. Editing a types file elsewhere has no effect. Check the server's active mission del tuo mod. |
...
| `cfgGameplay.json` changes ignored | File not enabled or wrong location | Place the file in the mission del tuo mod. Set `enableCustomGameplay` to `1` in `serverDZ.cfg`. Restart the server (not just reload). |
| Class inheritance not working in config | `baseClass` misspelled or not loaded | The classe genitore must exist in the same or earlier addon. Check that `requiredAddons[]` includes the addon defining the classe genitore. |

---

## 9. Problemi di Persistenza

Problemi con il salvataggio e il caricamento dei dati tra i riavvii del server.
...
Processi diagnostici passo-passo per situazioni comuni "it doesn't work" di tipo

### "My mod doesn't work at all"

1. **Check the script log** for `SCRIPT (E)` errori. Fix the first error you find. (Section 2)
...
4. **Do scripts compile?** Look for errore di compilaziones in the RPT. Fix any syntax errori. (Section 2)
5. **Is there an entry point?** Hai bisogno di un `modded class MissionServer`/`MissionGameplay`, a registered module, or a plugin. Scripts without an entry point never run.
...
2. **Widgets exist but invisible?** Check sizes (must be > 0, nessun valore negativo). Check `Show(true)` is called. Check text/widget alpha is not 0.
...
## 11. Riferimento Rapido dei Comandi di Debug

Usa questi nella console di debug DayZDiag o negli strumenti di amministrazione.
...
## 12. Posizione dei File di Log

Sapere dove cercare e meta della battaglia.
...
### Leggere i Log in Modo Efficace
...
## 13. Dove Trovare Aiuto

Quando questa guida non risolve il tuo problema, queste sono le migliori risorse.

### Risorse della Comunita

| Risorsa | URL | Ideale Per |
|----------|-----|----------|
...
Studia questi mod per imparare pattern da modder esperti:

| Mod | Cosa Imparare |
|-----|---------------|
| **Community Framework (CF)** | Modulo lifecycle, RPC management, logging, managed pointers |
...
### Riferimento degli Script Vanilla

Il riferimento autorevole per tutte le classi e i metodi del motore:
...
### Checklist Rapida Prima di Chiedere Aiuto

Prima di postare in un forum della comunita o su Discord, raccogli queste informazioni:

1. **Cosa ti aspettavi** che accadesse
2. **Cosa e realmente accaduto** (messaggi di errore esatti, comportamento)
3. **Estratto del log degli script** (le righe `SCRIPT (E)` rilevanti, non l'intero log)
4. **Il tuo codice** (le righe section, not the entire mod)
5. **Cosa hai gia provato** (fa risparmiare tempo a tutti)
6. **Versione di DayZ e lista dei mod** (la compatibilita conta)
7. **Client o server** (specifica quale lato ha il problema)

---

## Indice Rapido dei Sintomi

Non riesci a trovare il tuo problema nelle sezioni sopra? Prova questo indice alfabetico.

| Sintomo (what you see) | Go to |
|-------------------------|-------|
...
| Member already definiscid | [Section 2](#2-script-errors) |
...
| Undefiniscid variable | [Section 2](#2-script-errors) |
| Variabile redeclaration | [Section 2](#2-script-errors) |
...
*Problema ancora irrisolto? Check the [FAQ](faq.md) for additional answers, the [Cheat Sheet](cheatsheet.md) for syntax reference, or ask in the DayZ Modding Discord.*
