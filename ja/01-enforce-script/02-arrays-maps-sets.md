...
[ホーム](../../README.md) | [<< 前： Variables & Types](01-variables-types.md) | **Arrays, Maps & Sets** | [次： Classes & Inheritance >>](03-classes-inheritance.md)

---

## はじめに

実際の DayZ Mod はデータのコレクションを扱います：プレイヤーのリスト、アイテムのインベントリ、プレイヤーIDからパーミッションへのマッピング、アクティブなゾーンのセット。Enforce Script はこれらのニーズに対応する3つのコレクション型を提供します：
...
**重要な違い：** `Insert()` does **not** update existing keys. `Set()` does. When in doubt, use `Set()`.
