# 第1.4章：Modded Class（DayZ Moddingの要）

[ホーム](../../README.md) | [<< 前： Classes & Inheritance](03-classes-inheritance.md) | **Modded Classes** | [次： Control Flow >>](05-control-flow.md)

---

## はじめに

**Modded class は DayZ Modding で最も重要な概念です。** 元のファイルを置き換えることなく、既存のゲームクラスの動作を変更できる仕組みです。Modded class がなければ、現在のような DayZ Modding は存在しません。
...
これが **常に `super` を呼び出す** ことが重要な理由です。 If Mod A does not call `super.Say()`, then the original `Say()` never runs. If Mod B does not call `super.Say()`, then Mod A's `Say()` never runs. One mod skipping `super` breaks the entire chain.
...
The most common use. バニラコードの前後に動作を追加します。
...
Extend the class with new data. Modded class のすべてのインスタンスにこれらのフィールドがあります。
...
通常の継承では `private` メンバーにアクセスできませんが、**modded class は private メンバーにアクセスできます** of the original class. This is a special rule of the `modded` keyword.
...
これは強力ですが、慎重に使用すべきです。 Private メンバーが private なのには理由があります --- DayZ のアップデート間で変更される可能性があります。
...
These are the classes that virtually every DayZ mod hooks into. それぞれが提供するものを理解することが不可欠です。

### MissionServer

専用サーバーで実行されます。 サーバーの起動、プレイヤー接続、ゲームループを処理します。
...
クライアントで実行されます。 クライアント側の UI、入力、レンダリングフックを処理します。
...
プレイヤークラス。 ゲーム内のすべての生存プレイヤーは `PlayerBase` のインスタンスです (or a subclass like `SurvivorBase`). このクラスを Mod するのがプレイヤーごとの機能追加の方法です。
...
すべてのアイテムの基底クラスです。 これを Mod すると、ゲーム内のすべてのアイテムに影響します。
...
グローバルゲームクラスです。 ゲームのライフサイクル全体を通じて利用可能です。
...
車両の基底クラスです。 すべての車両の動作を変更するために Mod します。
...
## `#ifdef` ガードによるオプション依存関係

Modが別のModをオプションでサポートする場合、プリプロセッサガードを使用します。 If the other mod defines a symbol in its `config.cpp` (via `CfgPatches`), you can check for it at compile time.
...
これはいくら強調しても足りません。 Every time you see a bug report that says "Mod X broke when I added Mod Y," the first thing to check is whether someone forgot to call `super`.
...
1. **常に `super` を呼び出す** --- 文書化された理由がない限り
