...
Enforce Script はメモリ管理に**自動参照カウント（ARC）**を使用します — 従来の意味でのガベージコレクションではありません。`ref`、`autoptr`、生ポインタの仕組みを理解することは、安定した DayZ Mod を作るために不可欠です。間違えると、メモリリーク（サーバーが徐々にRAMを消費しクラッシュ）か、削除済みオブジェクトへのアクセス（有用なエラーメッセージなしの即座のクラッシュ）を引き起こします。この章では、すべてのポインタ型、それぞれの使い分け、最も危険な落とし穴である参照循環の回避方法を説明します。
...
| ポインタ型 | キーワード | オブジェクトを生存させるか？ | 削除時にゼロ化？ | 主な用途 |
|-------------|---------|---------------------|-------------------|-------------|
...
すべてのオブジェクトには**参照カウント**があります -- the number of strong references (`ref`, `autoptr`, local variables, function arguments) pointing to it. カウントがゼロになると、オブジェクトは自動的に破棄されます デストラクタが呼ばれます。

**弱参照**（生ポインタ）は参照カウントを増加させません。 オブジェクトを生存させずに観察します。
...
**重要な違い：** An `array<SafeZone>` holds **weak** references. An `array<ref SafeZone>` holds **strong** references. If you use the weak version, objects inserted into the array may be immediately deleted because no strong reference keeps them alive.
...
> **DayZ Moddingの慣例：** Most codebases use `ref` for class members and omit `autoptr` for locals (relying on the implicit strong reference behavior). The CLAUDE.md for this project notes: "**`autoptr` is NOT used** -- use explicit `ref`." Follow whichever convention your project establishes.
...
参照循環は、2つのオブジェクトが互いに強参照（`ref`）を保持するときに発生します。 どちらのオブジェクトも削除されません。各オブジェクトが互いを生存させるためです。 これは DayZ Mod でメモリリークの最も一般的な原因です。
...
### 修正方法： One side must be a raw (weak) reference

一方を弱参照にすることで循環を断ちます。 The "child" should hold a weak reference to its "parent":
...
Enforce Script には従来のガベージコレクタはありません that periodically scans for unreachable objects. 代わりに、**決定論的参照カウント**を使用します：
...
[<< 1.7: Math & Vectors](07-math-vectors.md) | [ホーム](../../README.md) | [1.9: Casting & Reflection >>](09-casting-reflection.md)
