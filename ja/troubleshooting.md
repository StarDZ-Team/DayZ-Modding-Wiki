...
これらは、Modが表示されない、アクティブにならない、または起動時にゲームによって拒否される問題です。

| 症状 | 原因 | 修正方法 |
|---------|-------|-----|
| "Addon requires addon X" error at startup | Missing or incorrect `requiredAddons[]` entry | Add the exact `CfgPatches` class name of the dependency to your `requiredAddons[]`. Names are case-sensitive. [第 2.2](02-mod-structure/02-config-cpp.md). |
| Mod not visible in launcher | `mod.cpp` file is missing or has syntax errors | Create or fix `mod.cpp` in your mod root. It must contain `name`, `author`, and `dir` fields. [第 2.3](02-mod-structure/03-mod-cpp.md). |
| "Config parse error" on startup | 構文 error in `config.cpp` | Check for missing semicolons after class closings (`};`), unclosed braces, or unbalanced quotes. Every class body ends with `};`, every property ends with `;`. |
...
| Mod loads but nothing happens | Scripts compile but never execute | Check that your mod has an entry point: a `modded class MissionServer` or `MissionGameplay`, a registered module, or a plugin. Scripts do not run by themselves. [第 7.2](07-patterns/02-module-systems.md). |
...
| "Addon X is not signed" | Server requires signed addons | Sign your PBOs with your private key and provide the `.bikey` to the server's `keys/` folder. [第 4.6](04-file-formats/06-pbo-packing.md). |

---

## 2. スクリプトエラー

これらはスクリプトログに `SCRIPT (E):` or `SCRIPT ERROR:` lines.

| 症状 | 原因 | 修正方法 |
|---------|-------|-----|
| `Null pointer access` | Accessing a variable that is `null` | Add a null check before using the variable: `if (myVar) { myVar.DoSomething(); }`. This is the most common ランタイムエラー. |
| `Cannot convert type 'X' to type 'Y'` | Direct cast between incompatible types | Use `Class.CastTo()` for safe downcasting: `Class.CastTo(result, source);`. Never assume a cast will succeed. [第 1.9](01-enforce-script/09-casting-reflection.md). |
| `Undefined variable 'X'` | Typo, wrong scope, or wrong layer | Check spelling first. If the variable is a class from another file, ensure it is defined in the same or lower layer. `3_Game` cannot see `4_World` types. [第 2.1](02-mod-structure/01-five-layers.md). |
| `Method 'X' not found` | Calling a method that does not exist on that class | Verify the method name and check the 親クラス. You may need to cast to a more specific type first. Check vanilla scripts at `P:\DZ\scripts\` for the correct API. |
...
| `Redeclaration of variable 'X'` | Same variable name declared in sibling `else if` blocks | Declare the variable once before the `if`/`else` chain, then assign inside each branch. [第 1.12](01-enforce-script/12-gotchas.md). |
| `Member already defined` | Duplicate variable or method name in a class | Check for copy/paste errors. Each member name 一意でなければなりません within a class hierarchy (including 親クラスes). |
...
| 構文 error with no clear message | Backslash `\` or escaped quote `\"` in string literal | Enforce Script's CParser does not support `\\` or `\"`. Use forward slashes for paths (`"my/path/file"`). For quotes, use single-quote characters. [第 1.12](01-enforce-script/12-gotchas.md). |
| `JsonFileLoader` returns null data | Assigning the return value of `JsonLoadFile()` | `JsonLoadFile()` returns `void`. Pre-allocate the object and pass it by reference: `ref MyConfig cfg = new MyConfig(); JsonFileLoader<MyConfig>.JsonLoadFile(path, cfg);`. [第 6.8](06-engine-api/08-file-io.md). |
...
| No ternary operator support | Using `condition ? a : b` syntax | Enforce Script has no ternary operator. Use an `if`/`else` block instead. [第 1.12](01-enforce-script/12-gotchas.md). |
| `do...while` loop error | Using `do { } while(cond)` | Enforce Script does not support `do...while`. Use a `while` loop with a `break` condition instead. [第 1.12](01-enforce-script/12-gotchas.md). |
...
リモートプロシージャコールとクライアント・サーバー通信の問題です。

| 症状 | 原因 | 修正方法 |
|---------|-------|-----|
| RPC sent but never received | Registration mismatch | Both sender and receiver must register the same RPC ID. Verify the ID matches exactly on both client and server. [第 6.9](06-engine-api/09-networking.md). |
...
| Data not syncing to clients | Missing `SetSynchDirty()` | After changing any variable registered for synchronization, call `SetSynchDirty()` on the entity. Without it, エンジン does not broadcast changes. |
...
| RPC floods and server lag | Sending RPCs 毎フレーム or in tight loops | Throttle RPC calls with timers or accumulators. Batch multiple small updates into a single RPC. Use Net Sync Variables for data that changes frequently. |
...
GUIレイアウト、ウィジェット、メニュー、入力の問題です。

| 症状 | 原因 | 修正方法 |
|---------|-------|-----|
| Layout loads but nothing is visible | Widget size is zero | Check `hexactsize` and `vexactsize` values. Both must be greater than zero. Do not use negative sizes. [第 3.3](03-gui-system/03-sizing-positioning.md). |
...
| Text shows `#STR_some_key` literally | Stringtable entry is missing or file is not loaded | Add the key to your `stringtable.csv`. Check that the CSV is in your mod root and has the correct `Language,Key,Original` header format. [第 5.2](05-config-files/02-inputs-xml.md). |
...
| ScrollWidget content does not scroll | Content is not inside a WrapSpacer or child widget | ScrollWidget needs a single child (usually a `WrapSpacer` or `FrameWidget`) that is larger than the scroll area. Put your content widgets inside that child. [第 3.3](03-gui-system/03-sizing-positioning.md). |
...
| UI looks different at other resolutions | Using hardcoded pixel values | Use proportional sizing (`halign`, `valign`, `hfill`, `vfill`) instead of fixed pixel values. Test at multiple resolutions. [第 3.3](03-gui-system/03-sizing-positioning.md). |

---

## 5. ビルドとPBOの問題

Modのパッキング、バイナライズ、デプロイの問題です。
...
| ファイル patching changes not taking effect | Not using the diagnostic executable | ファイル patching only works with `DayZDiag_x64.exe`, not the retail `DayZ_x64.exe`. Launch with `-filePatching` parameter. |
...
| Addon Builder reports "no entry" warnings | Config references a property that does not exist | These warnings are usually non-fatal. Check that all `CfgVehicles` 基底クラスes exist. Missing entries in inherited configs cause cascading warnings. |
...
サーバーまたはクライアントのFPS低下、メモリ問題、遅い操作です。

| 症状 | 原因 | 修正方法 |
|---------|-------|-----|
| Low server FPS (below 20) | Heavy processing in `OnUpdate()` or per-frame methods | Use a delta-time accumulator to throttle work: only execute logic every N seconds. Move expensive operations to timers or scheduled callbacks. [第 7.7](07-patterns/07-performance.md). |
| Memory grows over time (メモリリーク) | `ref` reference cycles preventing garbage collection | When two objects hold `ref` references to each other, neither is ever freed. Make one side a raw (non-`ref`) reference. Break cycles in cleanup methods. [第 1.8](01-enforce-script/08-memory-management.md). |
...
| ログ file growing very large | Excessive `Print()` or debug logging | Remove or guard debug `Print()` calls behind `#ifdef DEVELOPER` or a debug flag. Large log files can slow disk I/O. |
...
| Attachments do not snap to item | Wrong `inventorySlot` names | Attachment slot names 正確に一致する必要があります between the parent item's `attachments[]` and the child item's `inventorySlot[]`. Names are case-sensitive. |
| Item damage zones not working | `DamageSystem` config mismatch with model | Each `DamageZone` name must match a named selection in the model's Fire Geometry LOD. Check with Object Builder. [第 6.1](06-engine-api/01-entity-system.md). |
...
| Item has wrong weight or size | `weight` and `itemSize[]` config values | `weight` is in grams (integer). `itemSize[]` defines the inventory grid slots as `{width, height}`. Check 親クラス values if inheriting. |
...
| `cfgGameplay.json` changes ignored | ファイル not enabled or wrong location | Place the file in the mission folder. Set `enableCustomGameplay` to `1` in `serverDZ.cfg`. Restart the server (not just reload). |
| Class inheritance not working in config | `baseClass` misspelled or not loaded | The 親クラス must exist in the same or earlier addon. Check that `requiredAddons[]` includes the addon defining the 親クラス. |

---

## 9. 永続化の問題

サーバー再起動時のデータ保存と読み込みの問題です。
...
4. **Do scripts compile?** Look for コンパイルエラーs in the RPT. Fix any syntax errors. (Section 2)
...
## 11. デバッグコマンドクイックリファレンス

DayZDiagデバッグコンソールまたは管理ツールでこれらを使用してください。
...
経験豊富なモッダーからパターンを学ぶために、以下のModを研究してください：

| Mod | 学べること |
|-----|---------------|
| **Community Framework (CF)** | モジュール lifecycle, RPC management, logging, managed pointers |
...
コミュニティフォーラムやDiscordに投稿する前に、以下の情報を集めてください：

1. **期待した動作**
2. **実際に起きたこと** (exact error messages, behavior)
3. **スクリプトログの抜粋** (the relevant `SCRIPT (E)` lines, not the entire log)
4. **あなたのコード** (the relevant section, not the entire mod)
5. **既に試したこと** (saves everyone time)
6. **DayZバージョンとModリスト** (compatibility matters)
7. **クライアントまたはサーバー** (specify which side has the problem)

---

## 症状別クイックインデックス

上のセクションで問題が見つかりませんか？このアルファベット順インデックスを試してください。
...
| ファイル patching not working | [Section 5](#5-build-and-pbo-issues) |
...
| 変数 redeclaration | [Section 2](#2-script-errors) |
...
*問題がまだ解決しませんか？ Check the [FAQ](faq.md) for additional answers, the [Cheat Sheet](cheatsheet.md) for syntax reference, or ask in the DayZ Modding Discord.*
