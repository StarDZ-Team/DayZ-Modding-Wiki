...
|--------|-----------|-------------|
| `GetHud` | `Hud GetHud()` | Zwraca the HUD instance (client only). |
| `GetWorldData` | `WorldData GetWorldData()` | Zwraca world-specific data (temperature curves, etc.). |
...
| `GetControlDisabledMode` | `int GetControlDisabledMode()` | Zwraca the current input exclusion mode. |
...
| `OnClientNewEvent` | `PlayerBase OnClientNewEvent(PlayerIdentity identity, vector pos, ParamsReadContext ctx)` | Creates + equips new character. Zwraca `PlayerBase`. |
...
| `EquipCharacter` | `void EquipCharacter(MenuDomyslnyCharacterData char_data)` | Iterates attachment slots, randomizes if custom respawn disabled. Calls `StartingEquipSetup()`. |
...
- **Constructor**: Destroys existing menus, creates Chat, AkcjaMenu, IngameHud, VoN state, fade timers, SyncEvents registration.
- **OnInit**: Guards against double init with `m_Initialized`. Creates HUD root widget from `"gui/layouts/day_z_hud.layout"`, chat widget, action menu, microphone icon, VoN voice level widgets, chat channel area. Calls `PPEffects.Init()` and `MapZnacznikTyps.Init()`.
...
## How to Hook: The modded class Wzorzec
...
### Hooking Chat Wiadomoscs (Server-Side OnEvent)
...
| `ChatWiadomoscEventTypID` | Client | Chat message received (`ChatWiadomoscEventParams`) |
...
## Real-World Przyklads

### Przyklad 1: Server Manager Initialization
...
### Przyklad 2: Player Data Loading on Connect
...
### Przyklad 3: Client HUD Creation
...
### Przyklad 4: Chat Command Interception (Server-Side)
...
### Przyklad 5: Starter Kit via StartingEquipSetup
...
### Wzorzec: Delegate to a Central Manager
...
[<< Poprzedni: Central Economy](10-central-economy.md) | **Mission Hooks** | [Nastepny: Akcja System >>](12-action-system.md)
