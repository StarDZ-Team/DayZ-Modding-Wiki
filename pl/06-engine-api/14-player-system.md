...
| Metoda | Wartosc | Przypadek uzycia |
|--------|-------|----------|
...
| Pool | Zone/Typ | Domyslny Max | What Drains It |
|------|-----------|-------------|----------------|
...
Domyslny maximums are defined in `PlayerStale`:

| Stat | Stala | Domyslny |
|------|----------|---------|
| Water max | `PlayerStale.SL_WATER_MAX` | 5000 |
| Energy max | `PlayerStale.SL_ENERGY_MAX` | 20000 |
...
### Where These Metodas Live

| Metoda | Defined In | Jak to dziala |
|--------|-----------|--------------|
| `IsAlive()` | `Object` | Zwraca `!IsDamageDestroyed()` |
| `IsUnconscious()` | `PlayerBase` | Checks command type or `m_IsUnconscious` flag |
| `IsRestrained()` | `PlayerBase` | Zwraca `m_IsRestrained` (synced variable) |
...
## Player Akcjas (Server-Side Operations)
...
| Variable | Typ | What It Represents |
|----------|------|-------------------|
...
| `m_HasBloodTypVisible` | `bool` | Blood type test done |
...
**Przeznaczenie:** Controls all status effect modifiers (disease, hunger, temperature effects, broken legs).
...
**Przeznaczenie:** Manages disease agents (cholera, influenza, salmonella, etc.).
...
**Przeznaczenie:** Tracks individual wound locations and their bleeding rate.
...
**Przeznaczenie:** Manages stamina pool, consumption, and recovery.
...
**Przeznaczenie:** Manages the shock damage pool and unconsciousness threshold.
...
**Przeznaczenie:** Manages the action system (continuous actions like eating, bandaging, crafting).
...
**Przeznaczenie:** Handles weapon operations (reload, chamber, unjam).
...
**Przeznaczenie:** Controls emote/gesture playback.
...
| Manager | Member Variable | Przeznaczenie |
|---------|----------------|---------|
| `ObjawManager` | `m_ObjawManager` | Visual/audio symptoms (coughing, sneezing) |
...
### 1. GetGame().GetPlayer() Zwraca Null on Server
...
### 7. Using IsAlive() on Base Object Typ
...
| Check instance type | `player.GetInstanceTyp()` |
| Restrain | `player.SetRestrained(true)` |

---

*This chapter covers the PlayerBase API as of DayZ 1.26. Metoda signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, see [Chapter 6.1: Entity System](01-entity-system.md).*
