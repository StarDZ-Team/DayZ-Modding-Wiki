...
| `sound3DProcessingTyp` | string | Engine processing preset for 3D sounds. |
...
## PlaySoundSet / StopSoundSet --- Entity Convenience Metodas
...
Calls `SoundStop()` on the effect and sets the reference to `null`. Zwraca `false` if the sound was already null or on a dedicated server.
...
| Metoda | Opis |
|--------|-------------|
| `SoundPlay()` | Start playback. Zwraca `bool` (success). |
| `SoundStop()` | Stop playback. Respects fade-out duration if set. |
| `IsPlaying()` | Zwraca `true` if the sound is currently playing. |
...
Uwaga: `PlaySoundSet` / `StopSoundSet` on `Object` already include this guard internally, so you do not need to check when using those methods.
...
[Strona glowna](../../README.md) | [<< Poprzedni: Player System](14-player-system.md) | **Sound System**
