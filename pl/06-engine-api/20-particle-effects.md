...
## Particle Architecture Przeglad
...
### Metoda 1: ParticleManager (Recommended)
...
### Metoda 2: Particle Static Metodas (Legacy)
...
**Wazne:** Particles belonging to a `ParticleManager` pool ignore auto-destroy flags --- the pool manages their lifecycle.
...
## EmitorParam --- Runtime Parametr Tuning
...
**Uwaga:** On legacy `Particle`, wiggle only works when the particle has a parent. On `ParticleSource`, it works in all cases.
...
### ParticlePropertiesFlagas

| Flaga | Opis |
|------|-------------|
| `NONE` | Domyslny, no special behavior |
...
Wzorzecs seen in vanilla DayZ and community mods:
...
| `ParticleAutoDestroyFlagas` controls particle lifetime | Ignored for particles managed by a `ParticleManager` pool --- the pool handles lifecycle |
...
### 3. Not Destroying EffectParticle Referencje
...
### 4. Using SetParametr on Null Particle Effect
...
- **Performance:** The global `ParticleManager` pool is limited to 10,000 slots (`ParticleManagerStale.POOL_SIZE`). Exceeding this creates "virtual" particles that wait for a slot to free up. Mods spawning many simultaneous particles (e.g., weather effects, contaminated areas with hundreds of emitters) should monitor pool usage and avoid exhausting it.
...
[Strona glowna](../../README.md) | [<< Poprzedni: Terrain & World Queries](19-terrain-queries.md) | **Particle & Effect System**
