...
| `Typ` | string | Class name of the EffectArea subclass to spawn (`ContaminatedArea_Static`) |
| `TriggerTyp` | string | Trigger class name (`ContaminatedTrigger`). Leave empty for no trigger |
...
> **Performance Ostrzezenie:** The maximum number of emitters is clamped to 1000 per zone. More emitters means worse performance. Keep `InnerPartDist` large enough to avoid exceeding this limit.

### Particle Poles (Pre-1.28, Legacy)
...
| `PPERequesterTyp` | Post-process effect class applied to the player's camera (`PPERequester_ContaminatedAreaTint`) |
...
> **Uwaga:** The JSON snippets below include comments for clarity. Real JSON files must not contain comments.
...
| Typ | Breadcrumbs? | EyeAccommodation | Przeznaczenie |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | Stala darkness deep underground. Eye accommodation is fixed at the configured value |
...
> **Wskazowka:** When using `UseRaycast: 1`, raise the breadcrumb position slightly off the floor (a few centimeters in the Y axis) to avoid the ray being blocked by the ground surface.
...
1. **Script weather state machine** --- override `WorldData::WeatherOnBeforeZmiana()` (see `4_World/Classes/Worlds/Enoch.c` for an example)
...
> **Uwaga:** You only need to include the phenomena you want to override. Omitted phenomena use engine defaults.
...
| float[] | `wetnessWeightModyfikators` | `[1.0, 1.0, 1.33, 1.66, 2.0]` | Item weight multipliers by wetness level: Dry, Damp, Wet, Soaked, Drenched |
...
| Typ | Parametr | Domyslny | Opis |
|------|-----------|---------|-------------|
| float | `sprintStaminaModyfikatorErc` | `1.0` | Stamina consumption rate during standing sprint |
| float | `sprintStaminaModyfikatorCro` | `1.0` | Stamina consumption rate during crouched sprint |
...
| float | `sprintSwimmingStaminaModyfikator` | `1.0` | Stamina consumption during fast swimming |
| float | `sprintLadderStaminaModyfikator` | `1.0` | Stamina consumption during fast ladder climbing |
| float | `meleeStaminaModyfikator` | `1.0` | Stamina consumed by heavy melee and evasion |
| float | `obstacleTraversalStaminaModyfikator` | `1.0` | Stamina consumed by jumping, climbing, vaulting |
| float | `holdBreathStaminaModyfikator` | `1.0` | Stamina consumption when holding breath |
...
| bool | `allowRefillSpeedModyfikator` | `true` | Allow ammo-type-based shock recovery modifier |
...
| Parametr | Domyslny | What It Disables |
|-----------|---------|------------------|
...
| `disallowedTypsInUnderground` | `["FenceKit", "TerritoryFlagaKit", "WatchtowerKit"]` | Item types prohibited underground (includes inherited) |
...
Przyklad: `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
To process `customString`, override `OnSpawnByObjectSpawner()` on the spawned item's script class. See `StaticFlagaPole` in vanilla scripts for an example where the custom string specifies which flag to spawn on the pole.

> **Performance Ostrzezenie:** Spawning a large number of objects through this system impacts both server and client performance. Use it for detail objects and small additions, not for large-scale world modifications.
...
- **Leave `TriggerTyp` empty** if you want a visual-only gas cloud with no player damage.
...
- **Overrides the script state machine.** If both `cfgweather.xml` and a scripted `WeatherOnBeforeZmiana()` override are active, the XML limits apply and the script operates within those limits.
- **Only one cfgweather.xml is loaded** per mission folder. Multiple weather mods cannot stack XML files.

### Gameplay Settings

- **One cfgGameplay.json per mission.** Wartoscs are global --- they affect all players on the server.
...
| System | File | Location | Przeznaczenie |
|--------|------|----------|---------|
...
[<< Poprzedni: Admin & Server Management](22-admin-server.md) | **World Systems**
