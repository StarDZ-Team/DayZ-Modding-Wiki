...
> Gdy cos pojdzie nie tak, zacznij tutaj. Ten poradnik jest zorganizowany wedlug **tego co widzisz** (objawu), nie wedlug systemu. Znajdz swoj problem, przeczytaj przyczyne, zastosuj rozwiazanie.

---

## Spis tresci

1. [Mod sie nie laduje](#1-mod-wont-load)
2. [Bledy skryptow](#2-script-errors)
3. [Problemy z RPC i siecia](#3-rpc-and-network-issues)
4. [Problemy z UI](#4-ui-problems)
5. [Problemy z kompilacja i PBO](#5-build-and-pbo-issues)
6. [Problemy z wydajnoscia](#6-performance-issues)
7. [Problemy z przedmiotami, pojazdami i encjami](#7-item-vehicle-and-entity-issues)
8. [Problemy z konfiguracja i typami](#8-config-and-types-issues)
9. [Problemy z persystencja](#9-persistence-issues)
10. [Schematy decyzyjne](#10-decision-flowcharts)
11. [Szybka referencja polecen debugowania](#11-debug-commands-quick-reference)
12. [Lokalizacje plikow logow](#12-log-file-locations)
13. [Gdzie szukac pomocy](#13-where-to-get-help)
...
| "Config parse error" on startup | Skladnia error in `config.cpp` | Check for missing semicolons after class closings (`};`), unclosed braces, or unbalanced quotes. Every class body ends with `};`, every property ends with `;`. |
...
| Skladnia error with no clear message | Backslash `\` or escaped quote `\"` in string literal | Enforce Script's CParser does not support `\\` or `\"`. Use forward slashes for paths (`"my/path/file"`). For quotes, use single-quote characters. See [Chapter 1.12](01-enforce-script/12-gotchas.md). |
...
| Game input is stuck / cannot move after closing UI | `ZmianaGameFocus()` calls are imbalanced | Every `GetGame().GetInput().ZmianaGameFocus(1)` must be paired with `ZmianaGameFocus(-1)`. Track your focus changes and ensure cleanup happens even if the UI is force-closed. |
...
| Slider does not respond to input | Missing script handler or wrong widget type | Ensure the slider widget has a `ScriptClass` assigned and that your handler processes `OnZmiana` events. Initialize the slider range in script. |
...
| Item has no inventory icon | Missing or wrong `inventorySlot` or icon config | Define the `picture` path in your config pointing to a valid `.paa` or `.edds` icon file. Check `rotationFlagas` for correct icon orientation. |
...
| Items spawn with wrong quantities | `quantmin`/`quantmax` values incorrect | Wartoscs are percentages (0-100) in `types.xml`, not absolute counts. `-1` means "use default". |
...
| Items disappear from storage | `lifetime` expired in `types.xml` | Increase `lifetime` for persistent items. Domyslny is often too short for base-building containers. Check `globals.xml` `cleanupLifetimeRuin` value. |
...
1. **Check the script log** for `SCRIPT (E)` errors. Rozwiazanie the first error you find. (Section 2)
...
4. **Do scripts compile?** Look for compile errors in the RPT. Rozwiazanie any syntax errors. (Section 2)
...
4. **Input stuck after closing UI?** `ZmianaGameFocus()` calls are imbalanced. Every `ZmianaGameFocus(1)` needs a matching `ZmianaGameFocus(-1)`. Check cleanup runs even on force-close.

---

## 11. Szybka referencja polecen debugowania
