...
- [Tips and Boas Práticas](#tips-and-best-practices)
...
| Propósito | Descrição |
|---------|-------------|
| **Script editing** | Sintaxe highlighting, code completion, and error checking for `.c` files |
...
| Module | Camada | EntryPoint | Propósito |
|--------|-------|------------|---------|
...
| `Método 'Foo' not found in class 'Bar'` | Wrong method name or class |
...
| File Type | Ação on Double-Click |
|-----------|----------------------|
...
| Problema | Profiler Sintoma | Correção |
|---------|-----------------|-----|
...
**Correção:** Mount P: first. Check **Workbench > Options** source directory. Verify `.gproj` FileSystem paths exist.

### No Code Completion

**Causa:** Project misconfigured -- Workbench cannot compile scripts.
**Correção:** Verify `.gproj` ScriptModules include vanilla paths (`scripts/1_Core`, etc.). Check Output for compiler errors. Ensure vanilla scripts are on P:.

### Scripts Do Not Compile

**Correção:** Check Output panel for exact errors. Verify all dependency mod paths are in ScriptModules. Ensure no cross-layer references (3_Game cannot use 4_World types).
...
**Correção:** Check `.gproj` FileSystem includes the directory where your files live. Restart Workbench after modifying `.gproj`.

### "Plugin Not Found" Errors

**Correção:** Verify DayZ Tools integrity via Steam (right-click > Properties > Installed Files > Verify). Reinstall if needed.

### Connection to DayZDiag Fails

**Correção:** Both processes must be on the same machine. Check firewalls. Ensure Script Editor module is open before launching DayZDiag. Try restarting both.

---

## Tips and Boas Práticas
...
7. **Check compiler output before launching.** If Workbench reports errors, the game will fail too. Correção errors in Workbench first -- faster than waiting for game boot.
...
## Referência Rápida: Keyboard Atalhos
...
## Referência Rápida: Launch Parâmetros
...
| Anterior | Up | Próximo |
|----------|----|------|
| [4.6 PBO Packing](06-pbo-packing.md) | [Part 4: File Formats & DayZ Tools](01-textures.md) | [4.8 Modelagem de Edifícios](08-building-modeling.md) |
