...
|--------|-----------|-------------|
| `GetHud` | `Hud GetHud()` | Retorna the HUD instance (client only). |
| `GetWorldData` | `WorldData GetWorldData()` | Retorna world-specific data (temperature curves, etc.). |
...
| `GetControlDisabledMode` | `int GetControlDisabledMode()` | Retorna the current input exclusion mode. |
...
| `OnClientNewEvent` | `PlayerBase OnClientNewEvent(PlayerIdentity identity, vector pos, ParamsReadContext ctx)` | Creates + equips new character. Retorna `PlayerBase`. |
...
| `EquipCharacter` | `void EquipCharacter(MenuPadrãoCharacterData char_data)` | Iterates attachment slots, randomizes if custom respawn disabled. Calls `StartingEquipSetup()`. |
...
- **Constructor**: Destroys existing menus, creates Chat, AçãoMenu, IngameHud, VoN state, fade timers, SyncEvents registration.
- **OnInit**: Guards against double init with `m_Initialized`. Creates HUD root widget from `"gui/layouts/day_z_hud.layout"`, chat widget, action menu, microphone icon, VoN voice level widgets, chat channel area. Calls `PPEffects.Init()` and `MapMarcadorTypes.Init()`.
...
## How to Hook: The modded class Padrão
...
## Server vs Client: Which Hooks Fire Onde
...
### Exemplo 1: Server Manager Initialization
...
### Exemplo 2: Player Data Loading on Connect
...
### Exemplo 4: Chat Command Interception (Server-Side)
...
### Exemplo 5: Starter Kit via StartingEquipSetup
...
### Padrão: Delegate to a Central Manager
...
[<< Anterior: Central Economy](10-central-economy.md) | **Hooks de Missão** | [Próximo: Sistema de Ações >>](12-action-system.md)
