...
[<< Предыдущая: Timers & CallQueue](07-timers.md) | **File I/O & JSON** | [Следующая: Networking & RPC >>](09-networking.md)
...
> **Важно:** Most file write operations are restricted to `$profile:` and `$saves:`. Attempting to write elsewhere may silently fail.
...
> **Важно:** `FindFile` returns just the file name, not the full path. You must prepend the directory path yourself when processing the files.
