...
Every DayZ mod needs an entry point ----  place where it initializes managers, registers RPC handlers, hooks into player connections, and cleans up on shutdown. That entry point is the **Mission** class. The engine creates exactly one Mission instance when a scenario loads: `MissionServer` on a dedicated server, `MissionGameplay` on a client, or both on a listen server. These classes provide lifecycle hooks that fire in a guaranteed order, giving mods a reliable place to inject behavior.

Эта глава охватывает the full Mission class hierarchy, every hookable method, the correct `modded class` pattern for extending them, and real-world examples from vanilla DayZ, COT, and Expansion.
...
- **OnUpdate**: Выполняется tick scheduler, processes logout timers, updates base environment temperature, rain procurement, random artillery.
...
| `StartingEquipSetup` | `void StartingEquipSetup(PlayerBase player, bool clothesChosen)` | **Empty in vanilla** ---- выr entry point for starter kits. |
...
| `VONОстановитьSpeakingEventTypeID` | Client | Player stopped speaking |
...
### Пример 1: Server Manager Initialization
...
### Пример 2: Player Data Loading on Connect
...
Every `override` **must** call `super`. Forgetting it breaks vanilla and every other mod in the chain. Это single most common modding mistake.
...
- **Always call `super` as the first line in every Mission override.** Это single most common DayZ modding mistake. Forgetting `super.OnInit()` silently breaks vanilla initialization and every other mod in the chain.
...
- **Load Order:** The last-loaded mod's `modded class` override runs outermost in the call chain. If a mod forgets `super`, it silently blocks all mods loaded before it. Это #1 cause of multi-mod incompatibility.
