...
1. **inputs.xml** (config layer) --- declares named actions, assigns default keybindings, and organizes them into groups for the player's Controls settings menu. См. [Chapter 5.2: inputs.xml](../05-config-files/02-inputs-xml.md) for full coverage.
...
Эта глава охватывает the script layer: the classes, methods, and patterns you use to read and control inputs from Enforce Script.
...
- **`LocalPress()`** fires on exactly **one frame** ----  frame the key transitions from up to down. If you check it on any other frame, it returns false.
...
| Supports modifiers | Yes (Ctrl+Key combos via inputs.xml) | Ручное checking required |
...
Similar to ForceDisable but uses a different mechanism. Be cautious ---- если multiple systems lock/unlock the same input, they can interfere with each other:
...
When opening a full-screen UI, disable all game inputs except the ones your UI needs. Это pattern used by COT and Expansion:
...
**Важно:** Always call `GetUApi().UpdateControls()` after modifying input states in bulk.
...
For complete inputs.xml syntax, см. [Chapter 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).
...
`LocalPress()` returns `true` for exactly one frame ----  frame the key transitions from released to pressed. If your code path does not execute on that exact frame, you miss the event.
...
*Эта глава охватывает the script-side Input System API. For the XML configuration that registers keybindings, см. [Chapter 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).*

---

## Лучшие практики

- **Always use `UAInput` via inputs.xml for player-facing keybindings.** Это позволяет players to rebind keys, shows actions in the Controls menu, and supports gamepad input. Reserve `OnKeyPress` for debug shortcuts only.
