...
> **Official sample:** `DayZ-Samples/Test_Crafting/` demonstrates this exact pattern ---- n `ExampleRecipe` that turns any item into stones using a custom `MagicHammer`, registered via a modded `PluginRecipesManagerBase`.

### Шаг 3: No Further Wiring Needed
...
Called from the `RecipeBase` constructor. Set all field values here. Do not call `super.Init()` ---- он is an empty declaration in the base class.
...
- `uid` --- Override the crafting animation when this specific ingredient is used. Например, `CleanWeapon` uses `CMD_ACTIONFB_CLEANING_WEAPON` when the ingredient is a `DefaultWeapon`.
...
This is how the `RepairWithTape` recipe can repair almost any item ---- онs ingredient 2 accepts `Inventory_Base`, `DefaultWeapon`, and `DefaultMagazine`.
...
Это action that drives all recipe-based crafting. Players do not interact with it directly ----  `CraftingManager` creates it automatically when valid recipes exist between two items.
...
1. **WalkRecipes()** ---- онerates all registered recipes and maps each ingredient class name to recipe IDs with bitmask positions (`Ingredient.FIRST = 1`, `Ingredient.SECOND = 2`, `Ingredient.BOTH = 3`).
...
Это означает if you register `"Inventory_Base"` as an ingredient, every item that inherits from `Inventory_Base` gets that recipe in its cache automatically.
...
- `ExampleRecipe` extends `RecipeBase` ----  "MagicHammer" that turns any item into 4 small stones
...
- Recipe IDs are assigned sequentially at registration. Do not hardcode recipe IDs ---- y change when the load order changes.
- Two mods registering recipes for the same ingredient combination will both work ----  player sees all valid recipes and can cycle through them.
...
| `4_World/classes/useractionscomponent/actions/continuous/actionworldcraft.c` | `ActionWorldCraft` ----  crafting action |
