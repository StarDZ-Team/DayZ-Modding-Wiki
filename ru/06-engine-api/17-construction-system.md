...
- `4_World/entities/itembase/basebuildingbase.c` ----  entity base class
- `4_World/classes/basebuilding/construction.c` ----  construction manager
...
On completion, calls `BuildPartServer()` and damages the tool (`UADamageApplied.BUILD`). Both `ActionConditionПродолжить()` and `OnFinishProgressServer()` perform collision checks via `IsCollidingEx()` to prevent building through players or geometry.
...
Это raiding mechanism: explosives and weapons deal damage to damage zones until they reach ruined state. Barbed wire attachments have special handling --- when their zone is ruined, the wire's mounted state is cleared.
...
6. **Enable debug logging** via `LogManager.IsBaseBuildingLogEnable()` for diagnostics ----  vanilla code has extensive `[bsb]` prefixed debug output.
