...
| `STANCEIDX_RAISED` | 3 | Offset --- dd to base stance to get raised variant |
...
Example ---  greeting emote supports additive playback in erect/crouch and full-body in prone:
...
2. **Forgetting that action commands replace movement.** `StartCommand_Action()` is full-body ---  player stops moving. If you want an overlay animation, use `AddCommandModifier_Action()` instead.

3. **Using `SetAnimationPhase()` with wrong source names.** The string must match the `AnimationSources` class name in `model.cfg` exactly. There is no error if the name is wrong --- нетthing happens.
