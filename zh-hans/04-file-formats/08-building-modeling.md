...
本章涵盖 the complete workflow for adding interactive doors and climbable ladders to custom building models, based on official Bohemia Interactive documentation.
...
Almost all doors in vanilla DayZ are **120 x 220 cm** (width x height). Using these standard dimensions ensures animations look correct and characters fit through openings naturally. Model your doors **closed 默认情况下** and animate them to the open position -- Bohemia plans to support doors opening in both directions in the future.
...
Unlike the Arma series, DayZ does **not** require a `ladders[]` array in the game config class. 但是, two things are still necessary:

1. Your model must have a **config representation** -- a `config.cpp` with a `CfgVehicles` class (the same 基类 used for doors; see the door config section above).
...
1. **Model doors closed 默认情况下.** Animate from closed to open. Bohemia plans to support opening doors in both directions, so starting from closed is future-proof.
