...
本章涵盖 the complete particle pipeline: the `ParticleList` registry, both creation approaches, the `EmitorParam` system for runtime tuning, the `EffectParticle` wrapper, `SEffectManager` integration, and real-world patterns from vanilla code.
...
`ParticleSource` auto-destroys 默认情况下 when the particle ends or stops:
...
`EffectParticle` extends the `Effect` 基类 to provide a managed lifecycle for particles, integrated with `SEffectManager`. It wraps a `Particle` or `ParticleSource` internally.
...
Always clean up effects to prevent 内存泄漏s:
...
| `scripts/3_game/particles/particlebase.c` | `ParticleBase`, `ParticleEvents` --- 基类 and event system |
...
- **Multi-Mod:** `ParticleList` is a single global class. Multiple mods can register particles via `modded class ParticleList`, but particle filenames 必须是唯一的 --- duplicates cause an error on the second registration, and `GetParticleIDByName()` only returns the first match.
...
[Home](../../README.md) | [<< 上一章: Terrain & World Queries](19-terrain-queries.md) | **Particle & Effect System**
