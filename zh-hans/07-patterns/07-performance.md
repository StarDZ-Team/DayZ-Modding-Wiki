...
[<< 上一章: Event-Driven Architecture](06-events.md) | [Home](../../README.md)

---

## 简介

DayZ runs at 10--60 server FPS depending on player count, entity load, and mod complexity. Every script cycle that takes too long eats into that frame budget. A single poorly-written `On上级date` that scans every vehicle on the map or rebuilds a UI list from scratch can drop server performance noticeably. Professional mods earn their reputation by running fast --- not by having more features, but by implementing the same features with less waste.
...
- [上级date Rate Limiting](#update-rate-limiting)
- [Caching](#caching)
...
- [ ] On上级date operations are throttled by timer or batch size
