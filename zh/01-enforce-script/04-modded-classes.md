# Chapter 1.4：Modded 类 (The Key to DayZ Modding)
...
Methods like `On上级date()` and `CommandHandler()` run every tick or every frame. Adding expensive logic here destroys server/client performance:
...
### 6. Destructors: Clean 上级 Before `super`
