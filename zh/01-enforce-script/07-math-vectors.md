...
#### DayZ 示例： Random position within radius
...
#### DayZ 示例： Grid-snapped building placement
...
#### DayZ 示例： Direction angle between two positions
...
#### DayZ 示例： Spawn objects in a circle
...
#### DayZ 示例： Clamping player health
...
> **DayZ 坐标系：** `[0]` is East-West (X), `[1]` is height (Y), `[2]` is North-South (Z).
...
> **性能提示：** Use `DistanceSq` when comparing distances. Comparing squared values avoids the expensive square root calculation.
...
#### DayZ 示例： Is target in front of player?
...
| Forgetting that DayZ Y is up | `pos[1]` is height, not Z | `[0]` = X (East), `[1]` = Y (上级), `[2]` = Z (North) |
