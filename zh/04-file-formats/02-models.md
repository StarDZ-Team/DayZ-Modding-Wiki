...
[<< 上一章: Textures](01-textures.md) | **3D Models** | [下一章: Materials >>](03-materials.md)
...
- Set **Forward: -Z Forward**, **上级: Y 上级** in the FBX export dialog.
...
**修复：** Create a Geometry LOD with a simplified convex mesh. Assign mass to vertices.

### 2. Non-Convex Collision Shapes

**症状：** Physics glitches, objects bouncing erratically, items falling through surfaces.
**修复：** Break complex shapes into multiple convex components in the Geometry LOD. Each component must be a closed convex solid.

### 3. Inconsistent Named Selections

**症状：** Animations only work visually but not for collision, or shadow does not animate.
**修复：** Ensure every named selection that exists in the Resolution LOD also exists in Geometry, Fire Geometry, and Shadow LODs.

### 4. Wrong Scale

**症状：** Object is gigantic or microscopic in-game.
**修复：** Verify your 3D software uses meters as the unit. A DayZ character is approximately 1.8 meters tall.

### 5. Missing Memory Points

**症状：** Muzzle flash appears at the wrong position, attachments float in space.
**修复：** Create the Memory LOD and add all required named points at correct positions.

### 6. No Mass Defined

**症状：** Object cannot be picked up, or physics interactions behave strangely.
**修复：** Select all vertices in the Geometry LOD and assign mass via **Structure --> Mass**.
